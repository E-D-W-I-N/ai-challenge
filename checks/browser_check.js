...
async function routeChecks() {
  ...
  // вольна врать словом или брать не ту границу — проверке это не видно.
  {
    ...
    // дословно уезжают шесть. Меньшее из двух, как в `working_cover`: срез
    ...
    check("в краткосрочной — длина истории и сколько из неё уезжает дословно",
      /Сообщений в истории/.test(shown("#mem-short")) && /Уезжает дословно/.test(shown("#mem-short")) &&
        nums("#mem-short") === "8|6",
      shown("#mem-short"));
    check("и сказано, куда делось начало: память, а не окно и не сводка",
      /Остальные 2 — заменено рабочей памятью\./.test(shown("#mem-short")), shown("#mem-short"));
    ...
    check("памяти у него нет — не срезано ничего, и сказано именно это",
