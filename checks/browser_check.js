...
async function routeChecks() {
  ...
  // на все пять шагов, и ни один промпт, кроме первого, человек не видел.
  {
    ...
    // И каждый — **указание**: называет инструмент, которым его исполняют,
    // и не сообщает, что отметка уже стоит. Утверждение стоит на том, что
    // клиент **правда отправил**: сравни мы свои же константы, продуктовый
    // текст мог бы стать любым. Здесь уехали два перехода из трёх, третий —
    // в сцене потолка.
    check("отправленные продолжения — указания с именем инструмента",
      said.slice(1, 3).every((t) => t.includes("update_plan")) &&
        said[3].includes("finish_task"),
      JSON.stringify(said.slice(1)));
    check("и ни одно не сообщает, что шаг уже отмечен",
      !said.slice(1).some((t) => t.includes("отмечен") || t.includes("отмечены")),
      JSON.stringify(said.slice(1)));
  ...
  // ── потолок держит цепочку, и выход из него назван словами ──
  {
    ...
    // Третий переход, `validation>execution`: тоже указание с именем
    // инструмента и без вести о чужой отметке.
    check("и «исправляй» — такое же указание с именем инструмента",
      said[2].includes("update_plan") && !said[2].includes("отмечен"),
      said[2]);
