...
const PANEL_ROUTE = [
  ...
  // Выключатель памяти — тем же путём и при любом варианте обрезки: он
  // не про историю, а про слой поверх неё, и прятать его не за чем.
...
async function routeChecks() {
  ...
  // ── ведение памяти: та же карточка, другой служебный вызов ──
  //
  // Кадр служебного вызова один на оба, и назвать, чем занята пауза, может
  // только сервер: «Сворачиваю начало разговора…» под ведением памяти было
  // бы враньём ровно там, где строка состояния для того и заведена. Тем же
  // индексом подписана и врезка в просмотре промпта.
  //
  // Врезка приезжает своим слотом, без всякой стратегии: обрезка у чата —
  // «вся история», а память всё равно ведётся и всё равно встаёт в промпт.
  {
  ...
  // ── два служебных вызова на одном обмене: строка состояния меняет текст ──
  //
  // Память ведётся при любом варианте обрезки, и на обмене, где сводка
  // перевалила за порог, вызова честно два. Кадр приходит на каждый, и строка
  // состояния обязана назвать тот, который идёт сейчас: промолчи она про
  // второй — пользователь читал бы «Обновляю факты…» всё то время, пока
  // сворачивается начало разговора.
  {
    const FACTS = "[факты о разговоре]\nцель: собрать ТЗ\n[конец фактов о разговоре]";
    const { client, $, settle, Evt } = freshClient({
      delay: 60,
      facts: FACTS,
      service: { insert: "[пересказ начала разговора]\nбыло то-то", covered: 0, strategy: "summary" },
    });
    client.init();
    await settle(40);
    $("#input").value = "вопрос с двумя вызовами";
    $("#composer").requestSubmit();
    await settle(90);          // пришёл первый кадр: идёт ведение памяти
    const first = $("#feed").querySelector(".card-status");
    check("на первом кадре строка состояния говорит про ведение памяти",
      Boolean(first) && first.textContent.includes("Обновляю факты"),
      first && first.textContent);
    await settle(70);          // пришёл второй: пошло сворачивание
    const status = $("#feed").querySelector(".card-status");
    check("на втором она меняет текст, а не молчит про второй вызов",
      Boolean(status) && status.textContent.includes("Сворачиваю начало разговора"),
      status && status.textContent);
    check("и строка по-прежнему одна — второй кадр её не удвоил",
      $("#feed").querySelectorAll(".card-status").length === 1,
      "строк " + $("#feed").querySelectorAll(".card-status").length);
    await settle(400);
    const card = $("#feed").querySelector(".card");
    cardButton(card, "Показать промпт запроса").dispatchEvent(new Evt("click"));
    const roles3 = card.querySelector(".prompt-view")
      .querySelectorAll(".prompt-role").map((r) => r.textContent);
    check("и врезки в промпте стоят по порядку: рабочая память, потом сводка",
      JSON.stringify(roles3) === JSON.stringify([
        "системный промпт", "факты о разговоре", "сводка начала разговора",
        "сообщение пользователя",
      ]), JSON.stringify(roles3));
  }
  ...
  // врезок больше одной.
  {
    ...
    check("переключение памяти уехало PATCH'ем",
  ...
  // дословно»: окно с зажимом по прочитанному, окно, которому ведение ещё
  // ничего не прочитало, окно с выключенной памятью (зажима нет) и сводка.
  // Ветвь, которую не исполнило ни одно утверждение, вольна врать словом или
  // брать не ту границу — проверке это не видно.
  {
    ...
    const { client, server, $, settle, Evt } = freshClient({
      ...
      chats: [
        ...
        // Тот же выбор, что у соседа, но ведение ещё ничего не прочитало:
        // резать дальше прочитанного нельзя, а прочитано ноль — не режется
        // ничего. Число рядом с вариантом само по себе не значит ничего.
        { label: "без прочитанного", strategy: "window", keep_last: 2,
          transcript: talk.slice(0, 4) },
        // Память выключена — и зажима нет: чат, которому ничего не ведут,
        // ничего не теряет, отбрасывая начало. Заодно строка о том, едут ли
        // записи в промпт этого чата, есть у каждого положения выключателя.
      ...
      working: {
        "с памятью": {
          ...
          // Память отстала от истории: последнее ведение прочитало две
          // первые реплики. Хвост от этого уезжает длиннее просимых двух —
          // ровно так режет сервер (`working_cover`), и раздел обязан назвать
    ...
    // сообщений (шесть посева плюс обмен), окно просили в два, но ведение
    // прочитало только две первые реплики — отброшены ровно две, и дословно
    // уезжают шесть. Меньшее из двух, как в `working_cover`: срез по одному
    // хвосту унёс бы реплики, которых память никогда не видела, — и пропали
    // бы они разом и из запроса, и из памяти.
    ...
    check("ведение до него не дошло — не срезано ничего, и сказано именно это",
    ...
    check("записей у него своих нет, и об этом сказано словами",
      /Записей ещё нет/.test(shown("#mem-working")), shown("#mem-working"));
    ...
    check("и зажима по прочитанному у него нет: выключенная память его не держит",
    ...
    check("у чата с выключенной памятью сказано, что записи в его промпт не едут",
      /Память этого чата выключена/.test(shown("#mem-long")), shown("#mem-long"));
