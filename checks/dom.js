...
// сколько нужно, чтобы записать, с каким конфигом ушло сообщение.
//
// Правило стенда: **лучше упасть, чем соврать**. Где повторить браузер
// дёшево, стенд его повторяет, где нельзя — бросает с внятным текстом:
// тихо разошедшийся с браузером стенд даёт зелёное утверждение о сломанном.
...
class El {
  ...
  // точных пикселей это не даёт, важно лишь, что пустая лента не выше экрана.
  get scrollHeight() {
  ...
  // Прокрутка зажата между нулём и «дальше некуда», как в браузере: иначе
  // проверка поставила бы ленту туда, куда та не доезжает.
  get scrollTop() {
  ...
  // У <select> значение — это значение выбранного <option>. Значение, которого
  // среди опций нет, браузер не игнорирует: он снимает выбор совсем, и
  // `.value` пустеет. Иначе поле молча оставалось бы при старом значении.
  get value() {
  ...
  get textContent() {
    ...
    // В браузере textContent видит и положенное через innerHTML. Стенд
    // разметку не разбирает, поэтому снимает теги — иначе утверждение
    // о показанном тексте смотрело бы в пустоту.
  ...
  set innerHTML(value) {
    ...
    // его обратно в узлы не нужно. Список опций пересобран — выбор
    // возвращается к умолчанию, как в браузере.
    this._selectCleared = false;
    this.children.forEach((c) => (c.parentElement = null));
    this.children = [];
    this._text = "";
    this._html = String(value || "");
    // Браузер при подмене содержимого обнуляет прокрутку: без этого лента
    // «сохраняла» бы позицию, которой в браузере уже нет.
    this.scrollTop = 0;
  }
  ...
  // делегирование `change` на #panel-body, на втором — Escape.
  dispatchEvent(event) {
    if (!event.target) event.target = this;
    let node = this;
    while (node) {
      // Обработчики идут в порядке подписки, и инлайновый `on...` — такой же,
      // а не всегда последний: браузер их не переставляет.
  ...
  // Потеря фокуса, как в браузере: blur, следом focusout — и change, но
  ...
  // `next` — узел, **получающий** фокус: так браузер отличает «ушли
  // со страницы» от «щёлкнули по соседнему полю», и у `focusout` это
  // единственный способ узнать, что фокус остался внутри.
  blur(next) {
    ...
    // `focusout` всплывает — на этом держится правка записи памяти: поле
    // с текстом и список типов в одном блоке, и слушает он блок.
    this.dispatchEvent(new Evt("focusout", { relatedTarget: to }));
    if (changed) this.dispatchEvent(new Evt("change"));
  }

  // Лежит ли узел внутри этого — как `Node.contains`, включая «сам в себе».
  // Пустой аргумент — `false`: фокус, ушедший в никуда, внутри не остался.
  contains(node) {
    let walk = node || null;
    while (walk) {
...
// в единственном экземпляре: держим в общем списке ради порядка вызова.
const INLINE_EVENTS = [
...
// Комбинаторы он повторить не может и потому бросает: селектор, молча
// нашедший ноль, — зелёное утверждение о том, чего никто не проверил.
...
function buildDocument(html) {
  ...
  while ((match = tagRe.exec(html))) {
    ...
    // Достаётся он только листу: узел, у которого textContent собирается
    // из детей, подписывать нечем.
...
function buildServer(options) {
  ...
  const state = {
    ...
    reply: "ответ модели",
    // Числа usage у каждого ответа: словарь или функция (номер) → словарь.
    // Без них кадры отдавали бы `metrics: null`, и плитки всегда показывали
    // бы прочерк.
    usage: (options && options.usage) || null,
    // Упавший обмен: словарь или функция (номер) → null | описание падения.
    // Настоящая ошибка приходит кадром `error` с пустыми числами и следом
    // `done`; `done` можно погасить — поток вправе оборваться на ошибке.
    fail: (options && options.fail) || null,
    // Служебный вызов перед ответом: словарь или функция (номер) →
    // null | `{insert, covered, strategy}`. Задано — обмен идёт как на сервере
    // со сжатием: кадр `compressing`, потом `start` с врезкой вместо начала.
    // `strategy` называет вызов, как сервер; не задана — сводка.
    service: (options && options.service) || null,
    // Вызовы инструментов на обмене: список или функция (номер) → список
    // `{name, ok, message, plan}`. Кадр приходит на **исполненный** вызов,
    // и план в нём уже новый — как на сервере.
    tools: (options && options.tools) || null,
    // Обмен, отменённый **на сервере**: поток дочитывается до конца, а кадр
    // `done` несёт `cancelled: true` — так выглядит отмена от соседней вкладки.
    // Без этой формы «цепочка встала на отмене» держалось бы само собой.
    cancelled: (options && options.cancelled) || null,
    // Сколько миллисекунд ручка думает: число или функция (метод, путь) →
    // число. Нужен для окон: между запросом и ответом человек вправе открыть
    // другой чат, а без задержки такое окно через интерфейс ненаблюдаемо.
    lag: (options && options.lag) || 0,
    // Врезка рабочей памяти: готовая строка или null. Своя, а не часть
    // `service`: записи вписывает человек, и едут они при любой обрезке.
    // Служебного вызова за ней нет — и кадра `compressing` тоже.
    ...
    // Записи долговременной памяти: список на всю базу, как на сервере.
    // Врезка выше — готовая строка, а это то, что показывает вкладка.
    records: ((options && options.records) || []).map((seed, i) => ({
      seq: i + 1, at: i, ...seed,
    })),
    // Секрет, который сервер вырезает из уезжающего в базу (`redact`). Стенд
    // чистит тем же способом и только когда секрет задан: иначе «список
    // пополняется ответом ручки» ненаблюдаемо — тексты совпадали бы.
    ...
    // Рабочий слой чата: записи о задаче и сводки, отдельно от чата — как
    // на сервере. Записи материализуются один раз и дальше **живут**: собери
    // их заново в каждом ответе, правка исчезала бы к следующему чтению.
    working: {},
    models: [
  ...
  // Управление контекстом едет наружу тем же путём, что сэмплирование:
  // панель обязана отличать пустое окно («не режем») от нуля. Карта
  // «поле → умолчание»; у стратегии оно `full`, а не пустое.
  ...
  // **вычисляются** (`plan.stage_of`), и стенд считает их теми же правилами
  // и тем же порядком.
  ...
  // неутверждённый план со всеми шагами `done` подходит под оба сразу.
  ...
  // Этап чата прямо сейчас — или `null` при выключенном процессе. Одно место
  ...
  // План в виде, которым меряется продвижение, — «сделан ли» у каждого шага
  // и три флажка, тем же составом, что у `plan.advanced` на сервере. Стенд
  // **считает** продвижение сам, по плану до вызовов и после: подставь он
  // готовое `true`, «клиент продолжил на продвижении плана» стояло бы
  // на значении, которое стенд и выдумал.
  const planShape = (agent) => (agent.workflow === "plan"
    ? JSON.stringify([
      (((agent.plan || {}).steps) || []).map((s) => s.status === "done"),
  ...
  // серверный, директивой **для модели**: человеческий выбирает сам клиент.
  ...
  // пустое тело правки 400. Слой с разбором щедрее серверного прятал бы
  // дыру: форма осталась бы зелёной, получая в браузере 400.
  ...
  // Промпт стенд собирает в том же порядке, что сервер, и шлёт кадром `start`
  // всегда. Историю режет так же: иначе проверить, что кнопка промпта
  // показывает у окна уехавшее, было бы не на чем.
  ...
  // одним ответом. Слот не считается, а берётся из длины уже собранного
  // начала: сервер делает так же (`Agent.prompt_head`). Стенд от сервера
  // нарочно независим, но две копии формулы **внутри стенда** независимостью
  // не являются. Порядок тот же, что на сервере.
  ...
  // Кадр `start` — **один** на оба потока, удачный и упавший, и слоты в нём
  // не пересчитываются: их отдаёт та же сборка начала промпта. Слот упавшего
  // обмена через интерфейс ненаблюдаем, и вторую копию формулы не поймало
  // бы ничто.
  ...
  function sse(agent, text, signal) {
    ...
    // данными, а не догадкой клиента. Берутся по обе стороны от исполнения,
    ...
    // в историю не пишет, оборванный после — пишет начатое. Кадры вызовов
    // **не** откатываются никогда: исполненный вызов задним числом
    // не отменяется.
  ...
  // роняет ждущее чтение отказом `AbortError`, и стенд обязан так же: проглоти
  // он отмену, «Стоп» проверялся бы на потоке, который никто не рвал.
  // `onAbort` получает, сколько кадров успело уехать: обмен без единого куска
  // текста сервер в историю не пишет, и стенд обязан откатить записанное.
  ...
  async function fetchStub(path, init) {
    ...
    // Проверки границы те же, что на сервере: стенд, принимающий то, чего
    // сервер не принимает, оставляет зелёной форму, которая в браузере
    // получает 400. Тип записи обязателен и умолчания не имеет.
    ...
    if (planMatch && method === "POST") {
      ...
      // с правилом другого. Пауза — исключение, как на сервере: её нажимают
      // именно на работающем агенте.
      if (agent.busy && planMatch[1] !== "pause") {
