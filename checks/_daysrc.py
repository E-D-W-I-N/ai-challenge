...
Заготовленные чаты в нашем `day.py` собраны из этих колонок руками, и сверять
их глазами нельзя: двадцать одна колонка, у каждой полтора десятка полей
и промпты по полторы тысячи символов. Поэтому проверка берёт исходник
из ветки и сравнивает поле за полем.

`Scenario` и `Session` объявлены здесь заново, а не импортируются из
`app.schema`: сценариев больше нет, а `repeats` и `depends_on` ушли
