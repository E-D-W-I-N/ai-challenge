...
def main() -> int:
    ...
    # Системный промпт, поднятая из базы пара и новый вопрос. Блока задачи
    # здесь нет: чат обычный, «Рабочий процесс» у него выключен — и выключен
    # он и после переоткрытия файла, как всякое поле конфига.
    assert roles == ["system", "user", "assistant", "user"], roles
    assert not any("факты о разговоре" in m["content"] for m in second["prompt"]), second["prompt"]
