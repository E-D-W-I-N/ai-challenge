...
Проверка настоящая, а не «тот же объект в памяти»: скрипт запускает **два
отдельных процесса** питона, один за другим, и даёт им один файл базы. Первый
знакомится с агентом и умирает. Второй поднимается с нуля, продолжает ту же
сессию и должен отправить в модель вопрос вместе со всей прошлой историей.

Живых вызовов к модели нет: в обоих процессах `stream_completion` подменён
заглушкой, и она печатает наружу ровно то, что агент собрался отправить.
Именно по этому промпту и видно, что память вернулась из базы, а не из
воздуха: сам вопрос второго процесса ни имени, ни города не содержит.
"""
...
def _child(db: str, session: str | None, text: str) -> dict:
    """Один «запуск программы»: поднять агента, задать вопрос, напечатать итог.

    Выполняется в отдельном процессе — его и убивает перезапуск.
    """
