...
`ModuleNotFoundError: fastapi`, с ними даёт 7 проверок из 23 — нет текущего
...
```bash
.venv/bin/python checks/run_checks.py     # 23 проверки, включая три ниже и клиентскую
...
node checks/browser_check.js              # клиент под node, 114 утверждений
```
...
Бьют по поведению: ни таблиц выпиленных слов, ни проверок про проверки, ни самопроверок
стенда — таких не заводить. Исключений три, и все стерегут свойство продукта,
которого без браузера иначе не утвердить: «клиент ничего не тянет из сети» и «ключа нет
в клиенте» смотрят в текст статики, а `мимо redact() записать нельзя` выводит пути записи
из класса `Store` через `inspect`. Греп по клиенту не проверка клиента: `browser_check.js`
исполняет настоящий `app/static/app.js` на стенде `checks/dom.js` — минимальный DOM
и сервер, и ширина окна в нём переключается.
