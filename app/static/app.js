...
//
// Обмена кнопка не отправляет: человек переключил этап — значит, следующий
// же его вопрос уедет с новым блоком задачи. Отправлять за него ещё и
// сообщение значило бы говорить его голосом.
async function moveStage(token, label) {
  if (!(state.current && state.current.id)) return;
  try {
    const answer = await api(taskUrl() + "/stage", json("POST", { stage: token }));
    applyTask(answer.task, answer.task_log);
    taskStatus("Этап переключён: «" + label + "». Он уезжает в блок задачи каждого запроса.");
  } catch (err) {
