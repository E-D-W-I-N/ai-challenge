...
async function exchange(path, body, questionText, fromInput = false, agentId = null,
  ...
  // некому.
  //
  // Из двух условий работает обычно **второе**: оборванный поток кадра `done`
  // не доносит вовсе, и продолжать не от чего — цепочка после «Стопа» встаёт
  // раньше этой строки. `cancelled` тут **оборонительное**, как порядок двух
  // строк в `plan.stage_of`: окно между пришедшим `done` и закрытием потока
  // узкое, но в нём человек уже сказал «стоп», и слушаться надо его.
