...
// Цепочка: обмен кончился движением плана — следующий клиент шлёт сам.
//
// Сервер кончает обмен на первом же движении плана, и отсюда главное, чего
...
// на первой отметке нельзя — человек попросил сделать работу.
//
...
//
// Продолжаем, только если план **сдвинулся** (`plan_moved` — слово сервера,
// а не догадка) и переход **назван** в `STAGE_CHAIN`: не сдвинулся — цепочка
// встала, и это её главный ограничитель. Чат проверяется заново: между
// концом обмена и этой строкой лежит `refreshCurrent`, и человек вправе
// уйти в соседний чат.
