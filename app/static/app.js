...
// одно, а подпись врезки называла бы другое, и разошлись бы они молча. Какой
// шаг идёт, говорит сервер полем `strategy`; клиент про это не догадывается
// по тексту сообщений. Врезка рабочей памяти подписана здесь же, хотя вызова
// за ней нет вовсе: в промпте она стоит ровно так же, как стояла.
const SERVICE_CALLS = {
      ...
      (e) => {
        switch (e.event) {
          ...
          case "start":
            ...
            // в модель, и тот же, что стоит в блоке задачи.
          ...
          case "done":
            ...
            // Состояние задачи и журнал переходов — тем же кадром: этап
            // мог переключить человек из соседней вкладки, пока шёл ответ.
            if (e.task) {
...
// Рабочий процесс — закрытый список из двух положений, и пустого значения
// у него нет. Значение, которого в списке не оказалось (чат записан сервером
// другой версии), select снимает с выбора совсем — тогда показываем `off`:
// сервер такой конфиг читает так же, и панель обязана показывать то, что
// на самом деле произойдёт с чатом, а не пустое поле.
function fillWorkflow(value) {
  const select = $("#f-workflow");
...
function readPanel() {
  const patch = {
    ...
    // «Рабочий процесс» уезжает при любой стратегии: он не про историю,
    // а про то, ведёт ли чат задачу вообще.
...
function applySettings() {
  ...
  state.applying = (async () => {
    try {
      ...
      renderTiles();
      // «Рабочий процесс» видно сразу, а не со следующего сообщения: полоса
      // этапов это показ состояния, и чат, переставший вести задачу, обязан
      // остаться без неё тут же — ровно как остался без блока в промпте.
...
const TASK_LINE_LABELS = { step: "сейчас", expecting: "ожидается", rule: "правило" };

const TASK_DEFAULT_STAGE = TASK_STAGES[0][0];

// О чём просят на каждом этапе — та самая реплика, которую человек набирал
// бы руками сразу после переключения. Нажатие на полосу её и отправляет:
// этап без вопроса не делал ничего, и каждый переход стоил человеку одной
// и той же фразы.
//
// Просит она **результат**, а не согласие: «выполни и покажи», а не «готов
// ли ты приступить». Довод тот же, по которому его просит инструкция этапа
// (`TASK_STAGE_PLAN` на сервере), — на живых прогонах модель охотнее
// обещала, чем делала.
//
// Карта здесь одна и живёт на клиенте: это реплика **человека**, её никто
// больше не собирает и не показывает. Уезжает она обычным сообщением — той
// же ручкой, тем же кадром и в ту же ленту, что набранная руками.
const TASK_ASK = {
  planning: "Предложи план.",
  execution: "Выполни первый шаг плана и покажи результат работы.",
  validation: "Проверь сделанное и назови конкретные проблемы, если они есть.",
  done: "Подведи итог: что сделано, что осталось за рамками.",
};
...
function taskOf(agent) {
  ...
  return {
    ...
    lines: task.lines || [],
    // Правило этапа — то, что уехало **системным** сообщением: что на этом
    // этапе можно и нельзя. В строках блока его нет и быть не может — там
    // сведения, — но показать его человеку надо там же, где этап: это то,
    // что велено модели, и собрал его тот же сервер.
    rule: task.rule || "",
...
// Полоса этапов — **пульт**: она говорит, где задача стоит, и ею же этап
// переключают. Автоматики нет ни одной — этап двигает человек, нажимая
// на следующий, — но перепрыгнуть он не может: нажимается только то, куда
// пускает карта переходов, остальное приглушено и не нажимается.
//
// Запрет виден **до** попытки его нарушить, а не строкой отказа после. Карту
// при этом спрашивает сервер и присылает ответ полем `moves`: считай клиент
// достижимость сам — на экране стояла бы вторая карта переходов, и разошлась
// бы она с первой молча. Отбивает запрещённое всё равно сервер, даже когда
// полоса его не предлагала.
//
...
function renderStages(agent) {
  const box = $("#stages");
  box.innerHTML = "";
  // Чат, который задачу не ведёт, шапки не показывает вовсе: ни полосы,
  // ни строк под ней, ни журнала. Это то же самое, что происходит с ним
  // в промпте — блока задачи там тоже нет, — и два разных ответа на один
  // вопрос «есть ли тут задача» не заводим.
  const runs = Boolean(agent) && agent.workflow === "on";
  $("#chat-head").classList.toggle("hidden", !runs);
  if (!runs) {
  ...
  TASK_STAGES.forEach(([token, label], i) => {
    ...
    const reachable = task.moves.indexOf(token) >= 0;
    const mark = el(
      "button",
    ...
    // Недостижимое не нажимается по-настоящему, а не только выглядит
    // приглушённым: полоса обязана отказывать той же рукой, какой показывает.
    mark.disabled = !reachable;
    mark.title = token === current
      ? "Задача сейчас здесь"
      : (reachable ? "Перевести задачу сюда" : "Отсюда сюда перейти нельзя");
    if (reachable) mark.onclick = () => moveStage(token, label);
    box.appendChild(mark);
  });

  // Под полосой — то же, что уедет в промпт: чем заняты, чего ждут и что
  // на этом этапе велено. Первая строка блока здесь пропущена — это сам
  // этап, и он нарисован полосой выше. Собрал строки сервер: под полосой
  // обязано стоять ровно то, что видит модель.
  //
  // Правило дописывается последним и отдельным полем, потому что в промпте
  // оно едет отдельным сообщением — **системным**: сведения о задаче модели
  // сообщают, а правило ей велит. Человеком оно не правится вовсе: оно про
  // то, как вести себя на этапе, а не про эту задачу.
  const note = $("#stage-note");
  note.innerHTML = "";
  const rows = task.lines.slice(1);
  if (task.rule) rows.push(TASK_LINE_LABELS.rule + ": " + task.rule);
  rows.forEach((line, i) => {
...
// Перевод задачи на другой этап — единственная правка этапа, какая есть
// у продукта вообще. Своей ручкой, а не правкой текста: переход спрашивает
// карту, а правка полей её не знает.
//
// И нажатие **отправляет обмен** — тот, который на этом этапе и нужен
// (`TASK_ASK`). Раньше оно только двигало этап, и человек после каждого
// нажатия писал модели одно и то же руками. Реплика при этом остаётся
// его репликой: уходит обычной отправкой, встаёт в ленту пузырём вопроса
// и ничем не отличается от набранной — отправлена она нажатием, а не
// набором, вот и всё.
//
// **Порядок обязателен: сперва записать этап, потом отправить обмен.**
// Промпт собирает сервер на приходе запроса, и обмен, уехавший раньше
// записи, увёз бы в блоке задачи прежний этап — модель получила бы вопрос
// нового этапа с инструкцией старого. Поэтому отправка стоит за `await`
// ручки перехода, а не рядом с ней.
async function moveStage(token, label) {
  if (!(state.current && state.current.id)) return;
  // Идёт ответ — не трогаем ни этап, ни поток: оборвать чужой ответ нажатием
  // на полосу нельзя, а переключить этап, не отправив обмена, значило бы
  // сделать половину дела молча. Говорим, почему не вышло, и ждём конца.
  if (state.busy) {
    taskStatus("Идёт ответ. Дождитесь его конца и нажмите этап снова.", true);
    return;
  }
  let answer;
  try {
    answer = await api(taskUrl() + "/stage", json("POST", { stage: token }));
  } catch (err) {
    // Отказ сервера показываем его словами: карта переходов одна, и вторая
    // её копия, сочиняющая доводы на клиенте, разошлась бы с ней молча.
    // Обмена при отказе нет: этап не сменился, и спрашивать нечего.
    taskStatus(String(err.message || err), true);
    return;
  }
  applyTask(answer.task, answer.task_log);
  taskStatus("Этап переключён: «" + label + "» — и вопрос этого этапа уже ушёл.");
  const ask = TASK_ASK[token];
  // Ключа нет — отправки не бывает вовсе, ровно как у поля ввода: про это
  // говорит строка под ним, и второй раз тем же словами не повторяемся.
  if (!ask || !state.hasKey || !state.current) return;
  await exchange("/api/agents/" + state.current.id + "/messages", { text: ask }, ask);
}
...
function applyTask(task, log) {
  if (state.current) {
    state.current.task = task;
    // Журнал приезжает тем же ответом, что и состояние: строка появилась
    // от этого самого нажатия, и второй поход за ней оставил бы летопись
    // под полосой отставшей на один запрос.
    if (log) state.current.task_log = log;
  }
...
// Журнал этапа: когда и куда ушла задача — и ушла ли. Отклонённые попытки
// стоят здесь наравне с удавшимися: строка «проверка → планирование —
// отклонено» это доказательство, что карта работает, а не только нарисована.
// Строк «остались там же» здесь не бывает вовсе — их не пишет и сервер.
