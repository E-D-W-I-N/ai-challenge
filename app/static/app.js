...
//
// Все три — **указания**, а не сообщения: «шаг отмечен» модель читала как
// «отметку сделали за меня» и рисовала следующую словами. И все три —
// короткие: что делать и каким вызовом, сказано правилом этапа, а оно
// пересобирается перед каждым оборотом и потому свежее. Один приказ
// в двух местах — дефект: пересказ правила здесь стареет молча.
const STAGE_CHAIN = {
  "execution>execution": "Продолжай: следующий шаг.",
  "execution>validation": "Продолжай: проверяй работу.",
  "validation>execution": "Проверка нашла проблемы — исправляй.",
};
