...
//
// Сервер кончает обмен на первой же смене этапа: инструменты гаснут, модель
// договаривает словами. Отсюда главное, чего не было видно на живом прогоне, —
// у каждого этапа своя карточка в ленте, свой промпт и своя кнопка «Показать
// промпт запроса». Но бросать задачу на первом же переходе нельзя: человек
// попросил сделать работу, а не прислал по сообщению на этап.
//
// Ключ — **переход**, а не целевой этап, и это та же ошибка, на которой
// погибла выброшенная попытка дня: «шаги сделаны, проверяй» и «проверка нашла
// проблемы, исправляй» просят разного, а целевой этап у второго — тот самый
// `execution`, из которого мы только что ушли. Одной константой на оба модель
// делала бы работу заново вместо того, чтобы её исправить.
//
// Переходы, которые ждут человека, в таблице не стоят **вовсе**: `approval` —
// это кнопка «Утвердить план», `paused` — «Продолжить», `done` —
// «Переоткрыть». Молчание таблицы и значит «дальше ход человека»; второго
// списка запрещённых этапов рядом не заводим — две половины одного правила
// разошлись бы молча.
const STAGE_CHAIN = {
...
//
// Ограничитель у цепочки уже есть — она встаёт, едва этап перестал меняться, —
// но на кругах он не срабатывает никогда: модель, находящая проблемы каждый
// раз, гоняет `работа → проверка → работа` бесконечно, и платит за это человек.
// Четыре — это два полных круга: одного мало (первый же возврат из проверки
// упёрся бы в потолок), а дальше четвёртого продолжать незачем — модель ходит
// по кругу, и решать это человеку. Выход из потолка он обязан видеть словами:
// молча вставшая цепочка неотличима от сломанной.
...
//
// `chain` — который это обмен подряд из затеянных **самим клиентом** на одно
// сообщение человека. Счётчик едет параметром, а не лежит в `state`: цепочку
// рвёт что угодно — «Стоп», переключение чата, закрытая вкладка, — и счётчик
// в состоянии пришлось бы сбрасывать в каждом из этих мест поимённо, а забытое
// место молча продлило бы чужую цепочку. Параметр обнуляется сам: всякий обмен,
// затеянный человеком, начинается с нуля, потому что нуль тут умолчание.
async function exchange(path, body, questionText, fromInput = false, agentId = null,
      ...
      (e) => {
        switch (e.event) {
          ...
          case "done":
            ...
            // Отменён ли обмен — тоже знает агент, и у «отменён» **один**
            // признак на двух вестников: наш собственный оборванный поток
            // и слово сервера. Наш «Стоп» рвёт поток локально, и до этой
            // строки дело обычно не доходит — кадра `done` оборванный поток
            // не доносит вовсе. А вот отмена, поставленная соседней вкладкой
            // через `/cancel`, приезжает именно сюда, кадром: поток дочитан
            // до конца, признака отмены у клиента своего нет. Не прочитай
            // мы его — цепочка отправила бы следующий обмен **сама**, уже
            // после того, как человек попросил остановиться, и счёт за него
            // пришёл бы ему же.
  ...
  // Цепочку рвут «Стоп» и падение: продолжать то, что не доехало, значило бы
  // просить модель проверить работу, которой она не делала, — а счёт за это
  // придёт человеку. Переключение чата и закрытая вкладка рвут её сами:
  // первое проверено внутри `continueStage`, второе тем, что продолжать
  // некому.
  //
  // `cancelled` тут поднимается двумя путями, и второй — единственный, каким
  // до цепочки вообще доходит отмена. Наш «Стоп» рвёт поток, кадра `done`
  // не приходит, и цепочка встаёт раньше этой строки просто потому, что
  // продолжать не от чего. А отмена, поставленная снаружи — соседней вкладкой
  // через `/cancel`, — приезжает кадром `done` на дочитанном до конца потоке,
  // и останавливает цепочку ровно эта строка.
...
// не сменился — цепочка встала, и это её главный ограничитель; сменился
// на тот, что ждёт человека, — в таблице такого перехода нет, и мы молчим.
//
// Чат проверяется заново: между концом обмена и этой строкой лежит
// `refreshCurrent`, а пункт списка слева всё это время жив, и человек вправе
// уйти в соседний чат. Уедь продолжение как ни в чём не бывало — оно попало бы
// в чужую ленту.
