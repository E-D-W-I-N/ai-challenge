...
// Цепочка: обмен кончился продвижением плана — следующий клиент шлёт сам.
//
// Сервер кончает обмен на первом отмеченном шаге, и отсюда главное, чего
// не было видно на живом прогоне: у каждого шага своя карточка в ленте,
// свой промпт и своя кнопка «Показать промпт запроса». Но бросать задачу
// на первом сделанном шаге нельзя — человек попросил сделать работу.
//
...
// Третий переход — шаг внутри работы: этап тот же, а просят следующего шага.
//
...
//
// Все три — **указания**, и каждое называет инструмент, которым его
// исполняют. Сообщение вместо указания модель дочитывает за себя: «шаг
// отмечен» она читала как «отметку сделали за меня» и рисовала следующую
// словами.
const STAGE_CHAIN = {
  "execution>execution":
    "Возьмись за следующий шаг: отметь его in_progress через update_plan, " +
    "сделай работу и тем же вызовом поставь ему done.",
  "execution>validation":
    "Перечитай результат по каждому шагу и позови finish_task, передав " +
    "списком найденные пробелы.",
  "validation>execution":
    "Исправь названные проверкой проблемы, начиная с возвращённого в работу " +
    "шага, и отметь его done через update_plan.",
};

// Сколько обменов подряд клиент вправе затеять сам на одно сообщение человека.
// Первый ограничитель — цепочка встаёт, едва план перестал двигаться, — но
// работающая модель двигает его каждым обменом, а платит человек. Двенадцать:
// `update_plan` просит 3–7 шагов, и это семь шагов, круг проверки и запас
// на одну правку по её итогам. Выход из потолка человек обязан видеть
// словами: молча вставшая цепочка неотличима от сломанной.
const MAX_CHAIN = 12;
...
async function exchange(path, body, questionText, fromInput = false, agentId = null,
      ...
      (e) => {
        switch (e.event) {
          ...
          case "done":
            ...
            // Сдвинулся ли план — **данными**: отметка шага этап не меняет,
            // и по паре этапов она неотличима от обычного ответа словами.
            moved = e.plan_moved === true;
  ...
  await continueChain(stageFrom, stageTo, moved, agent.id, chain);
}

// Следующий шаг — следующий обмен, и затевает его клиент.
//
// Продолжаем, только если план **продвинулся** (`plan_moved` — слово
// сервера, а не догадка) и переход **назван** в `STAGE_CHAIN`: не
// продвинулся — цепочка встала, и это её главный ограничитель. Чат
// проверяется заново: между концом обмена и этой строкой лежит
// `refreshCurrent`, и человек вправе уйти в соседний чат.
async function continueChain(from, to, moved, id, chain) {
  const say = moved && from && to ? STAGE_CHAIN[from + ">" + to] : null;
  if (!say) return;
  if (!state.current || state.current.id !== id || !planOn(state.current)) return;
  if (chain >= MAX_CHAIN) {
    hint("Задача прошла " + MAX_CHAIN + " обменов подряд сама — дальше не иду: " +
