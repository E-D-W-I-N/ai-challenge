...
class Agent:
    ...
    def working_cover(self, spec: AgentSpec | None = None) -> int:
        """Докуда ведение памяти дочитало историю — зажато её длиной.
        ...
        Зажим длиной истории — про перегенерацию: она снимает пару с конца,
        и `upto` оказывается за краем списка.
        ...
        """
        return min(self.working["upto"], len(self.history))
    ...
    async def update_working(
        ...
        История не трогается. Не удалось ведение (ошибка, пустой ответ, ни
        одной разобранной правки) — память остаётся прежней, и обмен уезжает
        как уехал бы: служебный вызов не вправе обрушить сам разговор.

        `upto` и метрики двигаются только вместе с применённой правкой —
        любого из слоёв. Вызов, не изменивший ничего, не двигает и границу
        прочитанного: она зажимает окно, и подвинуть её за ответ, которого мы
        не поняли, значило бы дать выбросить непрочитанное.
        """
        ...
        if not self.apply_edits(parse_edits(content)):
