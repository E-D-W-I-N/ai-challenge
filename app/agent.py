...
class Agent:
    ...
    def facts_cover(self, spec: AgentSpec | None = None) -> int:
        ...
        Считается не по `upto`, а по хвосту: в модель уезжают последние
        `keep_last` реплик, всё остальное заменено выпиской. `upto` — про то,
        докуда извлечение прочитало историю, и на состав промпта он не влияет.
        ...
        if spec.keep_last is None or not self.facts["items"]:
            return 0
        return max(0, len(self.history) - spec.keep_last)
    ...
    def facts_plan(self, spec: AgentSpec) -> int | None:
        ...
        Число зажато длиной истории: перегенерация снимает пару **с конца**,
        и `upto` после неё оказался бы за краем.
        """
        if spec.strategy != "facts" or spec.keep_last is None:
            return None
        return min(self.facts["upto"], len(self.history))
