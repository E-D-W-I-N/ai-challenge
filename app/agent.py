...
TASK_SHOWN_FIELDS = (*TASK_TEXT_FIELDS, "guide")
"""Строки блока задачи после этапа: «сейчас», «ожидается» и инструкция этапа.
Порядок тот же, в каком они встают в промпт: чем заняты, чего ждут, что
велено."""
...
def task_shown(task: dict) -> dict:
    ...
    Инструкция этапа человеком не правится вовсе: полем состояния она
    не является и в базе её нет. Она про то, как вести себя **на этом
    этапе**, а не про эту задачу, — меняется она вместе с этапом.
    ...
    return {
        name: (task.get(name) if name in TASK_TEXT_FIELDS and task.get(name)
               else plan.get(name, ""))
        for name in TASK_SHOWN_FIELDS
    }
...
def task_lines(task: dict) -> list[str]:
    ...
    Дальше — «сейчас», «ожидается» и инструкция этапа, и только
    **непустые**: пустое уехало бы строкой «ожидается: » и заняло бы место,
    не сказав ничего. Довод тот же, что у блока профиля.

    Инструкция едет последней строкой и здесь же, в блоке задачи, а не
    отдельным сообщением: она про ту же задачу и меняется вместе с этапом.
    Ею этап и перестаёт быть подписью — на «проверке» модель получает прямое
    «напиши вердикт первой строкой», и по этому вердикту автомат потом
    решает, куда идти дальше.
    """
    lines = [f"{TASK_LABELS['stage']}: {TASK_STAGE_LABELS.get(task['stage'], task['stage'])}"]
    shown = task_shown(task)
    lines += [
        f"{TASK_LABELS[name]}: {shown[name]}" for name in TASK_SHOWN_FIELDS if shown[name]
    ]
    return lines


def task_view(task: dict) -> dict:
    """Состояние задачи так, как его ждёт экран: само состояние плюс строки
    блока — те самые, что уедут в промпт.
    ...
    у нас одна на всех.
    """
    return {**task, "lines": task_lines(task)}


TASK_SYSTEM = (
    "Ты смотришь на разговор и называешь то, чего в нём не хватает. "
    "Маршрут задачи держит программа: ни этапов, ни переходов называть "
    "не нужно, оценок и похвалы тоже.\n"
    'Ответ — только объект вида {"problems": ["…"]}, без пояснений вокруг. '
    'Называть нечего — верни {"problems": []}.'
)
"""Системный промпт вызова, который двигает этап. Свой, а не `spec.system`:
чат просили отвечать собеседнику, а здесь просят присмотреться, и чужая
роль испортила бы ответ — ровно как у сжатия.

Этапов он не называет вовсе, и это главное правило дня: **модель отвечает
на вопросы о разговоре, код — на вопросы о процессе**. Куда идти с «проверки»,
решает карта переходов, а не модель; модель говорит только, чего в сделанном
не хватает."""

TASK_PROBLEMS_KEY = "problems"
TASK_FORMAT = {"type": "json_object"}
"""Служебный вызов отвечает **структурой**, а не текстом: объектом
`{"problems": ["экран оплаты не обрабатывает отказ банка"]}` — или
`{"problems": []}`, если назвать нечего.

Работа у вызова одна на обоих этапах: **назвать недостающее**. На «работе»
это то, что из ожидаемого ещё не сделано, на «проверке» — конкретные
проблемы в сделанном; чего именно не хватает, говорит вопрос этапа, а форма
ответа одна.

Почему не словами. Вопроса «этап завершён? да/нет» здесь нет намеренно:
на оценку модель склонна отвечать «да» — спроси «готово?», скажет «готово»,
и автомат проехал бы работу и проверку за два обмена, что бы в них ни было
сделано. Поэтому модель не одобряет, а **ищет**: выдумать одобрение легко,
выдумать конкретную недоделку — нет.

Почему не перечнем строк. Пустоту словами модель тоже не вернёт: вместо
пустого списка она напишет «всё сделано, замечаний нет», и код, сверяющий
на пустоту, не сработает никогда. Пустой массив однозначен, фраза — нет,
и это та же ловушка разбора фразы, только этажом выше.

`response_format` здесь **ставится**, хотя у сжатия он снимается, и
противоречия в этом нет: у сжатия просят связный пересказ, и объект его
испортил бы, а здесь объект и есть то, что нужно. Довод был про пересказ
и на перечень не переносится."""

TASK_ASKS = {
    "execution": (
        "Что из ожидаемого на этом этапе ещё не сделано? Верни объект "
        f'{{"{TASK_PROBLEMS_KEY}": [...]}}, по строке на каждую недоделку. '
        f'Ничего не осталось — верни {{"{TASK_PROBLEMS_KEY}": []}}.'
    ),
    "validation": (
        "Назови конкретные проблемы в сделанном. Верни объект "
        f'{{"{TASK_PROBLEMS_KEY}": [...]}}, по строке на каждую проблему. '
        f'Проблем нет — верни {{"{TASK_PROBLEMS_KEY}": []}}.'
    ),
}
"""Вопрос на каждом этапе, который вызов вправе двигать. Ключи те же, что
у `TASK_AUTO`, и это одна карта, а не две: этап, у которого вопроса нет,
и есть этап, на котором вызова не бывает."""

TASK_TAIL = 6
"""Сколько последних реплик показывают этому вызову. Хвост, а не вся история:
этап — про то, где разговор **сейчас**, и начало беседы на это не влияет,
зато стоит денег на каждом обмене. Последний ответ модели в хвост входит
всегда — недостающее ищут именно в нём."""

TASK_AUTO = tuple(TASK_ASKS)
"""Этапы, с которых задачу двигает служебный вызов, — те, у которых есть
вопрос.

Планирования здесь нет **намеренно**: переход «планирование → работа» значит
«план утверждён», а утверждение плана — явное действие человека, и
догадываться о нём по тексту разговора нельзя. Из планирования выводит ровно
одна дверь: кнопка «Утвердить план» рядом с полосой этапов. Заодно чат,
в котором задачу ещё не начали, не платит за служебный вызов ни на одном
обмене.

«Готово» здесь нет по другому доводу: с него карта не ведёт никуда, и
спрашивать не о чем."""


def build_task_prompt(task: dict, tail: list["Turn"], user_text: str) -> list[dict]:
    """Промпт вызова, который двигает этап: чем заняты, **чего ждут**, хвост
    разговора, новый вопрос — и просьба вернуть перечень недостающего.

    Строка «ожидается» здесь обязательна: вызов ищет то, чего не хватает,
    и без неё не с чем сравнивать — «чего не хватает до чего?» осталось бы
    без второй половины.

    Этап в промпте назван **словами** («сейчас проверяют сделанное»), а не
    токеном: токены — словарь программы, и модели он ни к чему. Спрашивают
    её ровно о том, что она видела в разговоре.
    """
    stage = task.get("stage", TASK_STAGE_DEFAULT)
    shown = task_shown(task)
    parts = [
        f"Сейчас в задаче: {shown['step'] or TASK_STAGE_LABELS.get(stage, stage)}.",
        f"Ожидается: {shown['expecting']}." if shown["expecting"] else "",
    ]
    if tail:
        parts.append(
            "Последние сообщения:\n"
    ...
    parts.append("Новый вопрос пользователя:\n" + user_text)
    parts.append(TASK_ASKS[stage])
    return [
...
def read_problems(text: str) -> list[str] | None:
    """Перечень недостающего из ответа вызова — или `None`, если ответа
    не было вовсе.

    Сперва объект: `{"problems": [...]}` — ровно то, о чём просили, и
    пустой массив здесь значит «назвать нечего» однозначно, без фраз
    и синонимов.

    Запасной путь обязателен: json-режим умеет не всякая модель OpenRouter,
    а отказать чату из-за этого нельзя. Не разобрался объект — считаем
    строки, начинающиеся с дефиса: так модели и пишут перечни. Ни одной
    такой строки — значит, перечислять было нечего.

    Пустой ответ — это `None`, а не пустой перечень, и разница здесь
    принципиальная: «я посмотрел, и всё в порядке» обязано отличаться
    от «вызов ничего не сказал». Не отличалось бы — оборванный ответ
    закрывал бы задачу.
    """
    ...
    try:
        ...
        loaded = None
    if isinstance(loaded, dict) and isinstance(loaded.get(TASK_PROBLEMS_KEY), list):
        return [
            str(item).strip()
            for item in loaded[TASK_PROBLEMS_KEY]
            if str(item).strip()
        ]
    return [
        line.lstrip("-–— ").strip()
...
def next_stage(stage: str, problems: list[str] | None) -> str | None:
    """Куда уводит перечень недостающего — или `None`, если никуда.

    Весь маршрут задачи живёт здесь, в коде, а не в ответе модели, и смотрит
    он на **факт**: назван ли хоть один пункт. Названы — этап не закончен:
    на «работе» остаёмся работать, на «проверке» возвращаемся **в работу**.
    Пусто — этап пройден: работа уходит на проверку, проверка закрывает
    задачу.

    Обратное ребро `проверка → работа` — главное из двух, и держится оно
    на улике: проверка, назвавшая конкретную проблему, не вправе оставить
    задачу закрытой.

    Ответа не было (`None`) — нет и перехода. Молчание не значит «всё
    в порядке».
    """
    if problems is None:
        return None
    if problems:
        return "execution" if stage == "validation" else None
    if stage == "execution":
        return "validation"
    if stage == "validation":
        return "done"
    ...
    if stage == "execution":
        return "validation"
    if stage == "validation":
        return "execution" if answers.get("faults") else "done"
    return None
...
class Agent:
    ...
    def _setup(
        ...
        """Журнал этапа: `{seq, stage_from, stage_to, who, metrics, at}` на
        каждое решение — и человека, и служебного вызова.

        Держится в памяти рядом со сводками и по тому же доводу: из него
        считается стоимость служебных вызовов, а ходить за ней в базу на
        каждый показ плитки значило бы превратить одну отрисовку в поток
        запросов.
        """
        ...
        if store is not None:
            ...
            if saved is not None:
                ...
                # перезапуск ровно как сама задача, а её стоимость входит
                # в итог по чату — потеряй мы журнал, итог после перезапуска
                # стал бы меньше.
    ...
    def set_task(self, values: dict) -> dict:
        ...
        Пишет сюда **человек**: кнопкой полосы этапов и двумя полями вкладки.
        Карта разрешённых переходов ему не указ, и это не упущение: карта
        заведена для служебного вызова, который полосы не видит и отвечает
        за прыжок разве что деньгами. Человек полосу видит, кнопку жмёт сам
        и вправе перехватить этап в любой момент — хоть из «готово» обратно
        в планирование.

        Смена этапа дописывает строку журнала с подписью «человек»: журнал
        обязан объяснять **любой** переход, а не только те, что сделал агент.
        Правка полей журнала не трогает — переходом она не является.
        """
        was = self.task.get("stage")
        ...
        if self.task.get("stage") != was:
            self._log_move(was, self.task["stage"], "human")
        return dict(self.task)

    def _log_move(self, stage_from, stage_to, who: str, metrics=None) -> None:
        ...
        совпадать с базой — из него считается стоимость служебных вызовов,
        и разъехавшись, плитка и файл назвали бы разные деньги. Довод и
        форма те же, что у записей рабочей памяти.
        """
        if self.store is not None:
    ...
    def move_stage(self, target: str, who: str = "agent", metrics=None) -> bool:
        """Уводит задачу на `target`, если карта переходов это разрешает.
        `False` — состояние осталось прежним.

        Единственная дверь для всех, кто двигает этап **не руками**:
        и служебный вызов, и разбор вердикта проверки идут сюда. Запрет
        поэтому один, а не два: две копии карты разошлись бы молча, и прыжок,
        отбитый в одном месте, прошёл бы в другом.

        Строка журнала пишется и тогда, когда переход отбит, — но с прежним
        этапом в обеих колонках: в ней лежат метрики вызова, который это
        решил, и вызов, чья цена нигде не записана, сделал бы экономию чата
        враньём. Правило то же, что у сводок: числа живут там же, где
        результат.
        """
        stage = self.task.get("stage", TASK_STAGE_DEFAULT)
        allowed = isinstance(target, str) and task_move_allowed(stage, target)
        moved = allowed and target != stage
        if moved:
            if self.store is not None:
                self.task = self.store.save_task(self.id, {"stage": target})
            else:
                self.task["stage"] = target
        self._log_move(stage, self.task["stage"], who, metrics)
        return moved
    ...
    def task_plan(self, spec: AgentSpec | None = None) -> str | None:
        """С какого этапа служебному вызову предстоит двигать задачу — или
        `None`, если вызова не будет.

        Условие одно: этап из тех, что вызов вправе двигать (`TASK_AUTO`).

        Их два — «работа» и «проверка». С «планирования» задачу двигает
        только человек кнопкой «Утвердить план»: утверждение плана это его
        решение, а не вывод из разговора. С «готова» карта не ведёт никуда.

        Отсюда само собой выходит и то, о чём раньше пришлось бы
        договариваться отдельно: чат, в котором задачу не начинали, стоит
        на планировании и за служебный вызов не платит ни разу. Пустой чат
        обязан оставаться дешёвым ровно так же, как он обязан оставаться
        без врезки.
        """
        spec = spec if spec is not None else self.spec
        stage = self.task.get("stage", TASK_STAGE_DEFAULT)
        return stage if stage in TASK_AUTO else None

    async def update_task(
        self, spec: AgentSpec, user_text: str, context_length: int | None = None
    ) -> None:
        """Спрашивает модель, где теперь стоит задача, и двигает этап.
        Зовётся из `ask` **до** сборки промпта, как сжатие: этот же обмен
        уже уедет с новым этапом, и полоса в шапке переедет раньше первого
        токена ответа.

        Спрашивают у модели ровно два «да/нет» — закончен ли этап и годен ли
        результат проверки, — а маршрут держит код (`next_stage`) и карта
        переходов (`task_move_allowed`). Модель не называет этапов и не
        выбирает дорогу: она судит о том, что видела в разговоре.

        Любой сбой значит одно и то же — **оставить как было**: ответ
        не разобрался, вопрос остался без ответа, запрещённый картой
        переход, пустой ответ, ошибка провайдера, падение вызова. Служебный
        вызов не вправе обрушить разговор и не вправе двигать задачу наугад:
        обмен уезжает ровно так, как уехал бы без него.

        Формат ответа здесь **ставится**, а не снимается, как у сжатия:
        вызов не идёт на экран собеседнику, мы получаем его целиком и
        разбираем, — и объект это ровно то, что нужно. У сжатия просили
        связный пересказ, и объект его испортил бы; довод был про пересказ
        и сюда не переносится. Стоп-строки сняты по-прежнему: чужая
        стоп-строка оборвала бы объект на середине.
        """
        stage = self.task_plan(spec)
        if stage is None:
            return

        asking = replace(spec, response_format=TASK_FORMAT, stop=None)
        prompt = build_task_prompt(
            self.task_items(), self.history[-TASK_TAIL:], user_text
        )
        ...
        try:
            stream = stream_completion(
                asking, prompt_override=prompt, context_length=context_length
            )
            async with contextlib.aclosing(stream):
                async for event in stream:
                    if event["type"] == "done":
                        ...
                        return
        except Exception:  # noqa: BLE001 — падает служебный вызов, обмен живёт
            return

        target = next_stage(stage, read_problems(content))
        if target is None:
            # Ответа не было или названы недоделки — но вызов
            # уже оплачен, и цена его записывается всё равно: строкой журнала,
            # у которой этап в обеих колонках один. Правило то же, что
            # у сводок.
            self._log_move(stage, stage, "agent", metrics)
            return
        # Последнее слово всё равно за картой, а не за маршрутом: два места,
        # решающих одно, разошлись бы молча, и прыжок, отбитый в одном,
        # прошёл бы в другом.
        self.move_stage(target, "agent", metrics)

    def service_plan(self, spec: AgentSpec) -> list[str]:
        """Какие служебные вызовы предстоят этому обмену: `task` — ведение
        этапа, `summary` — сжатие. Пустой список — обмен идёт прямо к ответу.
        ...
        Список, а не один вызов: их снова два, и на одном обмене они
        встречаются. Кадр `compressing` уходит на каждый и полем `strategy`
        называет, какой именно, — клиент по нему пишет строку состояния,
        а гадать по тексту сообщений он не вправе.

        Ведение этапа стоит первым, потому что первым и идёт: оно двигает
        задачу, а сворачивание — историю, и полоса в шапке обязана переехать
        раньше, чем начнётся долгая пауза на пересказ.
        """
        plan = ["task"] if self.task_plan(spec) is not None else []
    ...
    def usage_summary(self) -> dict | None:
        ...
        # Третий проход — по журналу этапа. Вызов, который двигает задачу,
        # тоже уехал в модель и тоже оплачен, и метрики его лежат там же,
        # где его результат, — в строке журнала. Строки человека метрик
        # не имеют вовсе: нажатие кнопки ничего не стоит.
    ...
    def as_dict(self, *, with_transcript: bool = False) -> dict:
        ...
        if with_transcript:
            ...
            data["task_log"] = [
                {name: item[name] for name in ("seq", "stage_from", "stage_to", "who", "at")}
                for item in self.task_log
            ]
        return data
        ...
        * ответ частичный (обрыв, отмена) → пишутся обе реплики, у ответа
        ...
        async with self._lock:
            ...
            for call in self.service_plan(spec):
                ...
                if call == "task":
                    await self.update_task(spec, user_text, context_length)
                else:
