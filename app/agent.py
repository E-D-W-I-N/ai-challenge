...
class Agent:
    ...
    def context_slot(self, spec: AgentSpec | None = None) -> int | None:
        ...
        Врезок две — сводка и факты, и стоят обе на одном месте: сразу за
        системным промптом. Чем именно занят слот, клиенту говорит стратегия
        в том же событии; окно не вставляет ничего и слота не занимает,
        поэтому у него здесь `None`, как и у полной истории.
        """
        spec = spec if spec is not None else self.spec
    ...
    async def ask(self, user_text: str) -> AsyncIterator[dict]:
        ...
        * ответ частичный (обрыв, отмена) → пишутся обе реплики, у ответа
        ...
        async with self._lock:
            ...
            # `summary_at` — место врезки в промпте, `None` — врезки в нём
            # нет. Сам промпт клиент и так получает; без этого числа он
            # различал бы врезку разбором текста, то есть повторял бы у себя
            # устройство `build_prompt` и расходился бы с ним молча. Чем
            # занят слот — сводкой или фактами, — говорит стратегия рядом:
            # подписывает роль в просмотре промпта тоже сервер, а не догадка
            # клиента по первой строке сообщения.
            yield {
                "type": "start",
