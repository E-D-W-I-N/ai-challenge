...
class Agent:
    ...
    def turn_tools(
        ...
        на последнем разрешённом (`MAX_TURNS`) и ничего после продвижения
        плана.
        ...
        **Одно названное место** на решение «объявлять ли»: у предела два
        ...
        в тишину. Третье условие — **один сделанный шаг, один обмен**:
        план продвинулся (`plan.advanced`) — инструменты гаснут, модель
        договаривает словами, и цикл кончается сам. Взятие шага в работу
        продвижением не считается: погасив вызовы на нём, отметить сделанное
        было бы нечем. Обрывать цикл на месте нельзя: модель вернула бы вызов
        и ни слова, и обмену нечего записать. `entry` — план на входе
        в обмен, `None` — сравнивать не с чем.
        """
        if turn >= MAX_TURNS:
            return None
        if entry is not None and taskplan.advanced(entry, self.plan):
            return None
    ...
    async def ask(self, user_text: str) -> AsyncIterator[dict]:
        ...
        при этом **один сделанный шаг**: план продвинулся, и следующий оборот
        идёт без инструментов, а `done` называет `stage_from`/`stage_to`
        и `plan_moved`.
        ...
        async with self._lock:
            ...
            if not committed and failure is None:
                ...
                and taskplan.advanced(entry_plan, self.plan),
            }
            if not committed:
