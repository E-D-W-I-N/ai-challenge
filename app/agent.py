...
WORKING_SYSTEM = (
    ...
    "«список + тип: содержимое» — завести новую запись;\n"
    "«список номер тип: содержимое» — переписать запись с этим номером;\n"
    "«список - номер» — убрать запись, потерявшую смысл.\n"
    "Тип в списке «задача» — одно из четырёх слов: цель, ограничение, решение, "
    "открытый вопрос. В списке «навсегда» — одно из трёх: о собеседнике, решение, "
    "факт.\n"
...
типами записей: «решение» бывает и тем и другим, и отличает их только срок
...
молча, и «о собеседнике: пишет на Kotlin» легло бы в состояние задачи, а «цель»
осталась бы в памяти навсегда."""

EDIT_LAYERS = {TASK_MARK: "working", KEEP_MARK: "memory"}
"""Слово строки — в слой, к которому она относится. Типом строку не различить:
...
def _kind_words(labels: dict, kinds: tuple) -> dict:
    """Слово ответа — в тип записи. Принимаются и русская подпись, и токен:
    просим мы подпись, но модель вольна ответить тем словом, каким тип зовут
...
"""Типы по слоям — своя карта на каждый. Общей быть не может: `факт` в
...
def listing(records: list[dict], mark: str, labels: dict) -> str:
    """Список так, как его показывают служебному вызову: «слой номер тип:
...
def parse_edits(text: str) -> list[dict]:
    ...
    Неизвестный тип у новой записи — пропуск, а не подстановка умолчания:
    довод тот же, по которому его нет и у ручки (`_kind_field`, app/main.py).
    У правки тип назвать необязательно: «задача 12: новое содержимое» меняет
    текст, оставляя тип прежним.
    """
    out: list[dict] = []
    for raw in (text or "").splitlines():
        ...
        # Тип не назван — у правки это «оставить прежний», у новой записи
        # ничего: типа по умолчанию нет ни здесь, ни на ручке.
...
def build_working_prompt(
    ...
    пересказывает разговор, и то, что записано о собеседнике, в пересказе ему ни к чему.
...
class Agent:
    ...
    def apply_edits(self, edits: list[dict]) -> int:
        ...
        по типу записи: `решение` есть в обоих списках — оно бывает и про
        задачу, и про жизнь.
        """
        return (
    ...
    def apply_working_edits(self, edits: list[dict]) -> int:
        ...
        **Запись человека служебный вызов не трогает** — ни правкой, ни
        ...
        ронять из-за этого весь вызов незачем. Правка, не меняющая ни тип, ни
    ...
    def take_branch(self, carried: dict, *, parent_id: str, forked_at: int) -> None:
        ...
        # сквозными. Содержимое, тип, автор и время у копий прежние: ветка
