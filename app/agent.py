...
def build_facts_prompt(items: list[dict], chunk: list["Turn"], user_text: str) -> list[dict]:
    ...
    столько раз, сколько в нём сообщений.
...
def memory_lines(records: list[dict]) -> str:
    """Долговременная память строками «подпись рода: содержимое» — в том виде,
    в каком она уезжает в промпт. Род подписан по-русски и по одной карте
...
def merge_usage(total: dict | None, metrics) -> dict | None:
    """Складывает метрики служебных вызовов в один набор.

    Нужно фактам: снимок переписывается на каждом извлечении, и числа прошлых
    вызовов в нём не удержались бы — а заплачено за них так же, как за
    последний. Неизвестное поле пропускается, а не считается нулём.
    """
    ...
    for name in USAGE_FIELDS:
        ...
        if value is None:
            continue
        merged[name] = value if merged.get(name) is None else merged[name] + value
    return merged or None


CUT_METRIC = {"summary": "summarized", "window": "dropped", "facts": "facts"}
...
N сообщений», «факты вместо N сообщений». Слово на все случаи одно соврало бы
про какой-нибудь из них: сводка и факты начало **заменили** — и то и другое
видно в промпте запроса, — а окно его **отбросило** совсем. В суммы по чату ключ не
...
class Agent:
    ...
    def facts_cover(self, spec: AgentSpec | None = None) -> int:
        ...
        (`upto`). Резать только по хвосту нельзя: извлечение зовётся на каждом
        обмене, но проваливается молча — и тогда выписка остаётся вчерашней,
        а история растёт. Срез, считанный по одному хвосту, вырос бы вместе
        с ней и унёс реплики, которых выписка **никогда не видела**: под
        ответом было бы написано «факты вместо N сообщений» про то, чего
        в фактах нет. Ровно тот же порядок у сводки: `summary` срезает ровно
        то, что покрыла сводка, а не то, что хотелось бы срезать.

        Отсюда и цена провала: хвост уезжает длиннее просимого, пока
        извлечение не догонит историю. Это дороже, но честно, а молчаливая
        потеря начала разговора — нет.

        Фактов нет — не режется ничего: пустая выписка ничего бы не заменила,
        и «факты вместо десяти сообщений» под таким ответом были бы враньём.
    ...
    def memory_items(self, spec: AgentSpec | None = None, memory=None) -> list[dict]:
        ...
        Пусто здесь значит одно и то же в трёх случаях сразу: выключатель чата
        стоит в `off`, память пуста, хранилища у агента нет вовсе. Условие
        поэтому одно, а не три: **пустая память обязана быть неотличима
        от отсутствующей** — иначе чат на свежей базе получал бы в промпт
        пустую врезку, и всякая последовательность ролей, на которую смотрит
        любая проверка, сдвинулась бы на сообщение.
    ...
    def build_prompt(
        ...
        """Системный промпт + долговременная память + начало истории по
        стратегии + хвост + вопрос.
        ...
        Память встаёт **до** врезки стратегии и ни одну из них не отменяет:
        это разные слои — долговременный говорит, что мы знаем помимо этого
        разговора, а врезка стратегии заменяет начало самого разговора. Порядок
        от общего к частному: сперва то, что знали до чата, потом то, чем
        свёрнут чат, потом сам чат.
    ...
    def memory_slot(self, spec: AgentSpec | None = None, memory=None) -> int | None:
        """Каким по счёту сообщением промпта встала врезка долговременной
        памяти — или `None`, если её там нет.

        Ноль — законное значение: у чата без системного промпта память стоит
        нулевым сообщением. «Нет врезки» здесь только `None`, и сравнение
        на стороне клиента обязано быть строгим.
        """
        spec = spec if spec is not None else self.spec
        if not self.memory_items(spec, memory):
            return None
        return 1 if spec.system else 0

    def context_slot(self, spec: AgentSpec | None = None, memory=None) -> int | None:
        """Каким по счёту сообщением промпта встала врезка стратегии — или
        `None`, если её в нём нет вовсе.
        ...
        Врезок стратегии две — сводка и факты, и стоят обе на одном месте:
        сразу за системным промптом. Чем именно занят слот, клиенту говорит
        стратегия в том же событии; окно не вставляет ничего и слота
        не занимает, поэтому у него здесь `None`, как и у полной истории.

        А перед ними обеими может стоять третья врезка — долговременная
        память, слой не этого разговора. Она не отменяет ни одну стратегию
        и не заменяется ни одной, поэтому здесь её видно только сдвигом
        на единицу: врезок в промпте бывает две сразу.
        """
        spec = spec if spec is not None else self.spec
        if self.context_cut(spec)[1] is None:
            return None
        return (1 if spec.system else 0) + (1 if self.memory_items(spec, memory) else 0)
    ...
    def service_plan(self, spec: AgentSpec) -> str | None:
        ...
        или `None`, если обмен идёт прямо к ответу.

        Одна точка решения на оба вызова — и та же, что зовёт их. Событие
        `compressing` уходит по этому ответу, значит «сейчас будет пауза» и
        «пауза случилась» не могут разойтись: обещание строки состояния
        держится не совпадением двух условий, а тем, что условие одно.

        Стратегии исключают друг друга, поэтому и вызов за обмен бывает
        только один.
        """
    ...
    async def extract_facts(
        ...
        metrics: dict | None = None
        try:
            stream = stream_completion(asking, prompt_override=prompt, context_length=context_length)
            async with contextlib.aclosing(stream):
                async for event in stream:
                    if event["type"] == "done":
                        content = event["text"]
    ...
    async def ask(self, user_text: str) -> AsyncIterator[dict]:
        ...
        `error`, `done`. Первое приходит, только если этому обмену предстоит
        служебный вызов — сворачивание или извлечение фактов, — и раньше всех
        остальных: оно про паузу **до** ответа, и полем `strategy` называет,
        чем эта пауза занята.
        ...
        * ответ частичный (обрыв, отмена) → пишутся обе реплики, у ответа
        ...
        async with self._lock:
            ...
            # Служебный вызов — лениво и **до** сборки промпта: этот же обмен
            # уедет уже сжатым (или уже с обновлённой выпиской), и экономия
            # видна во входных токенах его собственного ответа, а не
            # следующего.
            #
            # Но сначала — событие о том, что вызов будет: он идёт к модели
            ...
            # паузу, которая будет, и говорит, из-за чего она.
            pending = self.service_plan(spec)
            if pending is not None:
                yield {"type": "compressing", "strategy": pending}
            if pending == "summary":
            ...
            # место врезки долговременной памяти; `None` у любого из них
            ...
            # расходился бы с ним молча. Чем занят слот стратегии — сводкой
            # или фактами, — говорит `strategy` рядом: подписывает роли
            # в просмотре промпта тоже сервер, а не догадка клиента по первой
            # строке сообщения.
            #
            # Слота два, потому что врезок в одном промпте бывает две:
            # память — слой не этого разговора, и стратегию она не отменяет.
            yield {
                "type": "start",
                "resolved_messages": prompt,
                "question": user_text,
                "memory_at": self.memory_slot(spec, memory),
                "summary_at": self.context_slot(spec, memory),
            ...
            # сводка и факты начало **заменили**, окно **отбросило** его.
            # Одно слово на все случаи соврало бы про какой-нибудь из них.
