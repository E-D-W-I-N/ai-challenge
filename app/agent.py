...
class Agent:
    ...
    def turn_tools(
        ...
        Четвёртым рядом стоит **принуждение** к вызову (`turn_choice`):
        оно спрашивает этот метод первой же строкой, потому что `tool_choice`
        без объявленных функций провайдер отвергает.
        """
        if turn >= MAX_TURNS:
            return None
    ...
    def turn_choice(
        ...
        """Принуждать ли модель к вызову на этом обороте — и к какому
        (`tool_choice` в теле запроса). Ничего не принуждать — `None`.

        Во всём дне правило одно: **инструкция в промпте — просьба, гарантию
        даёт код**. Утвердить себе план модель не может, работать на паузе
        не может, завершить неутверждённое не может — всё это держит
        `plan.apply`, а не текст. «На планировании сначала план, а не
        решение» держалось на уговорах, и живой прогон `openai/gpt-4o-mini`
        показал цену: на маленькой задаче модель пишет решение сразу. Она
        обучена выполнять просьбу, а не откладывать её, правило этапа стоит
        в начале промпта, а просьба человека — в конце. Поле закрывает это
        тем же способом, что и остальное: не словом, а кодом.

        Именно `update_plan`, а не `"required"` (`plan.FORCE_UPDATE_PLAN`):
        на планировании это единственное действие, которое `apply` не
        отказывает.

        **Решение живёт здесь, а не рядом с местом отправки**, и потому
        первой строкой спрашивает `turn_tools`: поле без объявленных функций
        провайдер отвергнет, и два разошедшихся места отправили бы
        `tool_choice` в оборот без `tools`. Отсюда сразу три следствия,
        которых не пришлось писать: на последнем разрешённом обороте
        (`MAX_TURNS`) принуждения нет, после смены этапа нет, при выключенном
        рабочем процессе нет.

        Что происходит по кругу: оборот 1 — принудительный `update_plan`,
        план записан, этап стал `approval`; оборот 2 идёт уже без инструментов
        (смена этапа) и, значит, без принуждения — модель договаривает
        словами. Вызов отказан (кривые аргументы, пустой список) — этап
        остался `planning`, и следующий оборот снова принудительный, и так
        до предела, где последний оборот всё равно без инструментов и модель
        отвечает текстом. Предел держит и этот случай.

        `stage` здесь тот же, что у `turn_tools`, — этап на входе в обмен;
        пройди он мимо первой строки, он уже равен нынешнему: несовпадение
        гасит инструменты.
        """
        ...
        * ответ частичный (обрыв, отмена) → пишутся обе реплики, у ответа
        ...
        async with self._lock:
            ...
            while True:
                ...
                # Принуждение к вызову решается **тем же** местом и на тех же
                # условиях: поле без `tools` провайдер отвергнет.
