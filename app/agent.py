...
def task_view(task: dict | None) -> dict:
    """Состояние задачи показанным: этап с подписью, шаг и ожидаемое действие —
    или пустой словарь.

    Функция, а не метод: тем же видом описывается чат, которого сейчас нет
    в памяти (список слева, `AgentRegistry.catalogue`). Второй копией этих
    трёх строк список отдавал бы про задачу не то же, что сам чат.
    ...
    return {
        ...
        "expects": task["expects"] or STAGE_EXPECTS[stage],
...
class Agent:
    ...
    def task_items(self) -> dict:
        ...
        Спрашивается одна строка состояния: поле конфига о том же расходилось
        бы с ней. Собирает показанное `task_view` — одним местом на врезку,
        на шапку и на список слева.
        """
        return task_view(self.task)
