...
class Agent:
    ...
    async def ask(self, user_text: str) -> AsyncIterator[dict]:
        ...
        `error`, `done`. Первое приходит на сворачивание — единственный
        вызов, который идёт к модели **до** ответа, — и раньше всех
        остальных: оно про паузу до первого токена, и полем `strategy`
        называет, чем эта пауза занята.
        ...
        * ответ частичный (обрыв, отмена) → пишутся обе реплики, у ответа
        ...
        async with self._lock:
            ...
            #
            # Вызов здесь снова один. Этап двигает не он, а инструмент внутри
            # самого обмена: отдельного обращения к модели у него нет, и
            # обещать паузу, которой не будет, незачем.
