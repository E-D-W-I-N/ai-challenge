...
# дальше путалась в собственном статусе и дёргала инструмент наугад —
# ровно это и увидел живой прогон.
#
...
def task_view(task: dict) -> dict:
    ...
    блока — те самые, что уедут в промпт, — и список достижимых отсюда
    этапов.
    ...
    переход разрешают или отбивают. Полоса этапов нажатием ничего больше
    не меняет и стала чистым показом: текущий выделен, достижимые видны
    обычными, недостижимые приглушены — запрет виден **до** того, как его
    попытались нарушить. Считай клиент достижимость сам, и на экране
    стояла бы вторая карта переходов.
    """
...
class Agent:
    ...
    def approve_plan(self) -> str:
        """Утверждение плана: единственный выход из планирования и
        единственный переход, который делает человек.

        Дверь узкая намеренно. «План утверждён» — решение человека, и
        выводить его из текста разговора нельзя: модель, дочитавшая свой же
        план, согласится с ним почти всегда. Все остальные переходы делает
        инструмент, и ни один из них сюда не ходит.
        """
        return self.move_stage("execution", "human")
    ...
    def move_stage(self, target: str, who: str = "agent") -> str:
        """Уводит задачу на `target`, если карта переходов это разрешает.
        Отдаёт исход — `moved`, `same`, `denied` или `unknown`.

        Единственная дверь всех переходов, и для инструмента, и для кнопки
        утверждения плана: две копии карты разошлись бы молча, и прыжок,
        ...
        * `same` — попросили тот этап, на котором и стоим. Это **не** ошибка
          ...
          зарос бы строками ни о чём, а модель решила бы, что сделала
          что-то не так, и принялась бы исправляться;
    ...
    def run_tool(self, call: dict) -> dict:
        """Исполняет вызов инструмента и отдаёт **запись о том, что решил
        код**: что просили (`name`, `arguments` — дословно, как пришли), чем
        это было (`outcome`, `stage_from`, `stage_to`), что ответили модели
        (`reply`) и что сказали человеку (`note`).

        Запись, а не пара «ответ, отказ»: тем же самым живёт журнал работы,
        и собирать её там заново значило бы разбирать текст ответа — то
        единственное, чего мы не делаем нигде. `note` по-прежнему только
        отказ: удавшийся переход виден полосой и строкой журнала, и
        приписывать его ещё и под ответом незачем.
        ...
        **Модель просит — код решает.** Проверка перехода стоит ровно там,
        ...
        """
        stage = self.task.get("stage", TASK_STAGE_DEFAULT)
        # Имя и аргументы — дословно, как пришли: журнал показывает то,
        # о чём просила модель, а не наш пересказ её просьбы.
        record = {
            "name": call.get("name"),
        ...
        if call.get("name") != TASK_TOOL_NAME:
            # Исходов по-прежнему четыре: чужое имя — то же «переходом
            # это не было», что и чужой этап, и в журнал переходов оно
            # тоже не идёт. Чем именно оно было, говорит ответ модели.
            return {**record, "reply": f"нет такого инструмента: {call.get('name')!r}"}

        target = read_stage_argument(record["arguments"])
        if target is None or target not in TASK_STAGES:
            ...
            return {**record, "stage_to": target, "reply": tool_reply("unknown", stage, target or "")}
        ...
        if stage == TASK_STAGE_DEFAULT and target != stage:
            ...
            return {
                **record,
                "stage_to": target,
                "outcome": "denied",
                "reply": tool_reply("denied", stage, target),
                "note": deny_note(stage, target),
            }
        ...
        return {
            **record,
            "stage_to": target,
            "outcome": outcome,
            "reply": tool_reply(outcome, stage, target),
    ...
    def prompt_head(
        ...
        # про текущую задачу, оба живут в чате и умирают с ним. И едет он
        # **всегда**, при любом этапе: этап у задачи есть всегда, а модель
        # не видит ни полосы этапов, ни панели — только промпт. Условия
        # у этой врезки нет вовсе, и слот её поэтому никогда не пуст.
    ...
    async def compress(
        self, spec: AgentSpec, context_length: int | None = None
    ) -> AsyncIterator[dict]:
        ...
        Наружу отдаёт события отладки (`debug`): что уехало на пересказ и что
        с него вернулось — дословно и **по ходу**, а не после. Поэтому метод
        и генератор, а не корутина с копилкой: вызов к модели длится сколько
        угодно, и журнал, узнающий о запросе только вместе с ответом, показал
        бы оба разом и ровно тогда, когда смотреть уже незачем.
        """
        ...
        # Промпт вызова — дословно и до вызова: журнал показывает содержимое,
        # а не пересказ событий, и «что отправили на пересказ» знает только
        # эта точка. Ключа здесь нет и быть не может: он живёт в заголовке
        # запроса, а не в теле (`stream_completion`, app/llm.py).
        yield {"type": "debug", "step": "compress_request", "messages": prompt}
        ...
        yield {"type": "debug", "step": "compress_reply", "text": content, "metrics": metrics}
    ...
    def usage_summary(self) -> dict | None:
        ...
        Журнала этапа среди слагаемых нет: переходы делает инструмент
        внутри самого обмена, и своей цены у них не бывает — цена обоих
        оборотов уже сложена в метриках ответа.
    ...
    async def ask(self, user_text: str) -> AsyncIterator[dict]:
        ...
        События: `compressing`, `debug`, `start`, `reasoning`, `delta`,
        `metrics`, `error`, `done`.

        `compressing` — «шаг идёт прямо сейчас», и полем `strategy` он
        назван: `summary` у сворачивания, `tool` у вызова инструмента,
        `continue` у ожидания продолжения. Шагов три, потому что и пауз
        три: сворачивание идёт до ответа, а вызов и продолжение — посреди
        него, и про середину раньше не говорилось ничего. Шлётся он ровно
        тогда, когда шаг правда будет: строка состояния обещает паузу,
        которая случится.

        `debug` — **содержимое** шага, дословно: что уехало на пересказ
        и что с него вернулось, что просила модель вызовом и что решил код,
        что уехало вторым обходом. Клиент показывает это журналом работы;
        в базу не пишется ничего — журнал живёт в памяти вкладки. Ключа
        в этих кадрах нет и быть не может: он живёт в заголовке запроса
        (`stream_completion`), а не в теле.
        ...
        * ответ частичный (обрыв, отмена) → пишутся обе реплики, у ответа
        ...
        async with self._lock:
            ...
            #
            # Сворачивание — единственный шаг **до** ответа, и потому
            # единственный, про который решают здесь. Вызов инструмента
            # и ожидание продолжения — шаги середины обмена, и обещать их
            # заранее нельзя: будут они или нет, решит сама модель.
            if self.compress_plan(spec) is not None:
                yield {"type": "compressing", "strategy": "summary"}
                folding = self.compress(spec, context_length)
                async with contextlib.aclosing(folding):
                    async for event in folding:
                        yield event
            ...
            try:
                while True:
                    ...
                    # Шаг идёт **прямо сейчас** — и это видно: строка состояния
                    # заводится тем же событием, что и у сворачивания, и полем
                    # `strategy` называет, чем занята пауза. Событий о служебной
                    # работе стало три, потому что и шагов стало три: раньше
                    # обмен знал одну паузу до первого токена, теперь у него
                    # есть середина — вызов и ожидание продолжения, — и молчать
                    # о ней значило бы показывать пустую карточку там, где
                    # работа идёт.
                    yield {"type": "compressing", "strategy": "tool"}
                    for call in calls:
                        record = self.run_tool(call)
                        if record["note"]:
                            notes.append(record["note"])
                        # Что просила модель и что решил код — одной записью
                        # и сразу: собери её клиент из журнала переходов,
                        # он не узнал бы ни аргументов вызова, ни исходов
                        # `same` и `unknown` — переходами они не были, и
                        # строки в летописи у них нет.
                        yield {"type": "debug", "step": "tool", **record}
                        talk.append(
                            {
                                "role": "tool",
                    ...
                    # Продолжение — второе обращение к модели, и пауза перед
                    # ним такая же настоящая, как перед первым. Вместе со
                    # строкой состояния уезжает и то, **что** уехало: весь
                    # список сообщений следующего обхода, с вызовом и его
                    # результатом на своих местах. Кадр `start` этого показать
                    # не мог — он уехал до вызова.
                    yield {"type": "compressing", "strategy": "continue"}
                    yield {
                        "type": "debug",
                        "step": "continue",
                        "round": rounds,
