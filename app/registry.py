        ...
        for row in self.store.list_sessions():
            ...
            entries.append(
                spec_as_dict(
                    ...
                    # А план — не приезжает: `plan` здесь остаётся `None`,
                    # и это не пробел. Ключ в теле есть у обоих случаев
                    # (`spec_as_dict`), а второго запроса в базу на каждый
                    # чат списка нет — от него в своё время увели и `branch`.
                    # Состояние задачи показывает открытый чат, у которого
                    # план уже поднят в память.
                )
            )
