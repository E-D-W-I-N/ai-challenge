...
from .agent import SAMPLING_FIELDS, Agent, AgentBusyError, deny_note, task_view
...
# --- состояние задачи ---------------------------------------------------------
#
# Ручка под чатом, как у рабочей памяти: область состояния — сам разговор,
# и умирает оно вместе с ним. Одна, а не четыре: состояние это одна строка
# на чат, а не список записей, и править её по одной операции не из чего.
#
# Читать её отдельно незачем — состояние приезжает с самим чатом
# (`GET /api/agents/{id}`), потому что полоса этапов стоит в шапке и рисуется
# тем же ответом, что и лента. Второй источник того же числа разошёлся бы
# с первым, как разошлась бы стенограмма, отданная дважды.


@app.patch("/api/agents/{agent_id}/task")
async def patch_task(agent_id: str, payload: dict = Body(...)) -> dict:
    """Правка полей состояния: `{"step": "..."}`, `{"expecting": "..."}`
    или оба сразу.

    Этапа здесь нет: у перехода своя ручка (`POST .../task/stage`), и она
    спрашивает карту. Ручка правки текста второй дверью была бы — она карты
    не знает и пускала бы куда угодно, — поэтому `stage` для неё просто
    лишнее поле, и отвечает на него общий разбор: 400.

    «Сейчас» и «ожидается» правятся руками — это тексты, а не переходы:
    человек пишет ими, чем занят и чего ждёт, и зашитое умолчание этапа
    его текст не перебивает.

    Разбор тела — теми же общими помощниками, что у профиля и памяти:
    лишнее поле 400, пустое тело 400 (`_record_body`), пустая строка поле
    **снимает** (`_profile_field`). Названные поля записываются,
    неназванные не трогаются: вторая вкладка, правящая «ожидается», иначе
    затирала бы шаг, набранный в первой.
    ...
    блока задачи (`lines`) — те самые, что уедут в промпт, — и список
    достижимых этапов (`moves`): под полосой стоит ровно то, что видит
    модель, а сама полоса приглушает то, куда карта не пускает.
    """
    agent = _agent(agent_id)
    _record_body(payload, TASK_TEXT_FIELDS)
    if not payload:
        raise HTTPException(
            status_code=400,
            detail=f"тело правки пустое: назовите {', '.join(TASK_TEXT_FIELDS)} или часть",
        )
    values = {name: _profile_field(payload, name) for name in payload}
    return {"task": task_view(agent.set_task(values))}


@app.post("/api/agents/{agent_id}/task/stage")
async def move_stage(agent_id: str, payload: dict = Body(...)) -> dict:
    """Переключение этапа: `{"stage": "execution"}`. Этап двигает **человек**,
    нажимая на полосу в шапке, и это единственный механизм перехода.

    Карту спрашивает сервер, а не разметка. Полоса недостижимые этапы
    приглушает и не даёт по ним щёлкнуть, но «ассистент не может
    перепрыгнуть этап» обязано держаться **кодом**: запрос, пришедший мимо
    полосы — из другой вкладки, из curl, от клиента другой версии, —
    получает 409 и текст, объясняющий, куда отсюда можно (`deny_note`).
    Отклонённая попытка при этом ложится строкой в журнал переходов:
    улика того, что автомат работает, а не только обещает.

    Тип поля — обязательный и из закрытого списка, тем же валидатором, что
    у типа записи памяти (`_kind_field` с ним одного устройства): умолчания
    у этапа здесь нет, потому что «сервер выбрал за человека, куда двигать
    задачу» — это ровно то, чего в ручном управлении быть не должно.
    Незнакомый этап — 400 и перечень допустимых, а не 409: это не запрет
    карты, а слово, которого в ней нет вовсе.

    Свой этап — не ошибка и не переход: нажали туда, где и стоим, состояние
    то же, журнал молчит. Ответ тот же, что у удачного перехода: показывать
    нечего, менять нечего.

    Отдаёт ручка и **журнал переходов**: он стоит тут же, под полосой, и
    новая строка в нём появляется ровно от этого нажатия. Спроси его клиент
    отдельным запросом — летопись отставала бы от полосы на один поход
    к серверу.
    """
    agent = _agent(agent_id)
    _record_body(payload, ("stage",))
    target = payload.get("stage")
    if not isinstance(target, str) or target not in TASK_STAGES:
        raise HTTPException(
            status_code=400,
            detail=f"stage: одно из {', '.join(TASK_STAGES)}, а не {target!r}",
        )
    was = agent.task.get("stage")
    if agent.move_stage(target, "human") == "denied":
        raise HTTPException(status_code=409, detail=deny_note(was, target))
    return {"task": task_view(agent.task), "task_log": agent.task_log_view()}
