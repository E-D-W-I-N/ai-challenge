...
"""Чистый чат кнопки «Новый чат»: пусто всё, кроме модели — без неё запрос
некуда отправить. Системного промпта нет намеренно, его задаёт пользователь."""
...
async def _pump(
    ...
    вкладка иначе жжёт токены, а агент ещё и допишет недосмотренный ответ.

    `on_close` висит именно здесь. Отвались клиент до первого события — задача
    с генератором отменяется, не начав выполняться, и `finally` внутри
    `make_events()` не сработает никогда. `finally` этой функции выполнится
    при любом исходе: starlette её генератор либо дочитывает, либо закрывает.
    """
...
async def list_agents() -> dict:
    """Список слева: плоский, на чистом старте пустой — чаты заводит
    пользователь. Ключа здесь нет, наружу уходит только факт его наличия."""
    return {
        "has_key": has_key(),
        "live": len(REGISTRY),
...
async def patch_agent(agent_id: str, payload: dict = Body(...)) -> dict:
    ...
    # исказить не может — `Agent.ask` идёт на слепке конфига. Действует
...
def _reserve(agent: Agent) -> None:
    ...
    а не ошибкой внутри уже открытого потока."""
    try:
        agent.reserve()
...
def _restore_and_release(agent: Agent, taken) -> Callable[[], None]:
    ...
    Клиент мог отвалиться **до первого события**: тогда `_regenerate_events`
    не запускается вовсе и снятая пара пропала бы вместе с вопросом. То же
    место, что и у брони, — `finally` в `_pump`. `restore` сам проверяет,
    не занял ли место новый обмен, поэтому звать его дважды безопасно.
    """
