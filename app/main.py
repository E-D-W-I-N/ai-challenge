...
на старте процесса сохранённые сессии поднимаются обратно в память, и список
слева после рестарта выглядит так же, как до него, вместе со всей перепиской.
"""
...
async def _lifespan(_app: FastAPI):
    # Сначала поднимаем сохранённое, потом добираем недостающее из ростера:
    # в обратном порядке агенты дней 1–5 задвоились бы на каждом запуске.
...
def ensure_roster() -> list[Agent]:
    ...
    Зовётся на старте процесса — после того, как сохранённые сессии подняты
    из базы, — и после «Очистить все чаты»: агенты дней 1–5 часть стенда,
    а не пользовательские чаты, и пропасть они не должны. Идемпотентность
    здесь и есть защита от дубликатов на втором запуске: агент, поднятый
    из базы, уже живой, и второй раз его никто не создаёт.
    """
