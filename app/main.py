...
async def patch_task(agent_id: str, payload: dict = Body(...)) -> dict:
    """Переключение этапа и правка полей: `{"stage": "execution"}`,
    `{"step": "..."}`, `{"expecting": "..."}` или любое их подмножество.
    ...
    **снимает** (`_profile_field`), а чужой этап — 400 с перечислением
    допустимых (`_choice_field`). Названные поля записываются, неназванные
    ...
    if not payload:
        raise HTTPException(
            status_code=400,
            detail=f"тело правки пустое: назовите {', '.join(TASK_FIELDS)} или часть",
        )
