...
async def patch_task(agent_id: str, payload: dict = Body(...)) -> dict:
    ...
    Этапа здесь нет, и это главное в устройстве дня: у каждого перехода
    ровно один механизм. Из планирования выпускает кнопка «Утвердить план»
    (`POST .../task/approve`), остальные три ребра проходит инструмент
    модели, и обе двери ведут в одну — `Agent.move_stage`, где стоит карта.
    Ручка правки текста третьей дверью была бы: она карты не спрашивала
    и пускала куда угодно. Поэтому `stage` здесь — просто лишнее поле,
    и отвечает на него общий разбор: 400.

    «Сейчас» и «ожидается» править руками по-прежнему можно — это тексты,
    а не переходы: человек пишет ими, чем занят и чего ждёт, и зашитое
    умолчание этапа его текст не перебивает.
...
@app.post("/api/agents/{agent_id}/task/approve")
async def approve_plan(agent_id: str) -> dict:
    """«Утвердить план»: планирование → работа, и подпись в журнале — «человек».

    Единственный переход, который делает человек, и единственный выход
    из планирования. Своя ручка, а не поле в правке: утверждение плана —
    не правка текста, а решение, и тело ей не нужно вовсе — она умеет ровно
    одно, и спорить с ней не о чем.

    Карту она всё-таки спрашивает, потому что идёт через ту же
    `Agent.move_stage`, что и инструмент: дверей две, а карта одна.
    Задача не на планировании — 409: переход не состоялся, и молчать об
    этом нельзя, иначе кнопка врала бы об успехе.
    """
    agent = _agent(agent_id)
    outcome = agent.approve_plan()
    if outcome != "moved":
        raise HTTPException(
            status_code=409,
            detail="утверждать нечего: задача не на планировании",
        )
    return {"task": task_view(agent.task)}
