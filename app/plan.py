...
STAGE_RULES = {
    ...
    "execution": (
        "Этап: выполнение, шаг {k} из {n} — «{title}». В этом обмене делается "
        "только шаг {k}: отметь его in_progress, выполни, отметь done. "
        "Следующий шаг — в следующем сообщении. План меняется только вызовом "
        "update_plan: написать в ответе «отметил шаг» или нарисовать список "
        "со статусами — значит не изменить ничего. Когда все шаги done — "
        "проверь работу и позови finish_task."
    ),
    "validation": (
...
это сведения. Правило работы называет **один** шаг, текущий: следующий
берёт первый оборот следующего обмена. Правило паузы названо жёстко («даже если тебя просят
продолжить»), но держится пауза не на нём — оба инструмента отказаны
кодом (`apply`)."""

FINAL_RULES = {
    "planning": "Скажи, что план составлен, и попроси человека утвердить его кнопкой.",
    "approval": (
        "Перечисли шаги коротко и попроси человека нажать «Утвердить план»; "
        "решение не пиши."
    ),
    "execution": "Расскажи, что сделано на шаге {k} — «{title}»; к следующему не переходи.",
    "validation": "Назови, что проверил и что нашёл.",
    "done": "Подведи итог сделанного.",
    "paused": "Скажи одной фразой: работа на паузе, снимает её человек кнопкой.",
}
"""Правило на обороте, где инструменты не объявлены, — таблицей и тем же
образцом, что `STAGE_RULES`, но **вместо** него, а не в придачу.

Полное правило там лжёт: оно велит звать то, чего в этом обороте нет,
и ни словом не говорит, каким быть ответу. Единственным выполнимым
оставалась исходная просьба человека — на живом прогоне модель на неё
и отвечала, минуя план. Поэтому здесь одно предложение на этап и только
про ответ."""

FINAL_COMMON = (
    "Вызовы в этом обороте недоступны: не обещай отметить или вызвать — "
    "этого не произойдёт."
)
"""Общая часть правила последнего оборота — одна на все этапы: обещанный
вызов не состоится, а на следующем обмене обещание лежит в истории
и читается как сделанное дело."""
...
def stage_rule(plan, final: bool = False) -> str:
    ...
    `final` — оборот идёт без инструментов, и правило **заменяется**
    коротким (`FINAL_RULES` плюс `FINAL_COMMON`): собирается оно всё равно
    в одном месте."""
    stage, current = stage_of(plan)
    rule = f"{FINAL_RULES[stage]} {FINAL_COMMON}" if final else STAGE_RULES[stage]
...
_TAILS = {
    "approval": "перечисли шаги человеку и попроси нажать «Утвердить план»",
...
Каждый говорит, **что делать дальше**. «Жди кнопки» на утверждении стоял
указанием молчать, и модель, дочитав его, отвечала на исходную просьбу
мимо плана."""
...
def _updated(plan: dict, args) -> tuple[dict, str]:
    ...
    `approved` не снимается. Запрещено у завершённой задачи и у списка
    короче утверждённого."""
    steps = _new_steps(args)
    # Список короче утверждённого — **отказ**, и состояние не меняется.
    # На живом прогоне модель слала по одному-два шага вместо четырёх, хотя
    # описание инструмента просит весь список. Снимать за это утверждение
    # пробовали: этап откатывался в `approval`, и ответ посреди работы
    # кончался просьбой нажать кнопку, а трёх шагов в плане уже не было.
    # Сравнивается **число** шагов: сверка по заголовкам читала бы
    # переименование как «удалили один, добавили другой». Только
    # у утверждённого: до утверждения переписывать список короче — это
    # и есть планирование.
    was = len(_steps(plan))
    if plan["approved"] and len(steps) < was:
        raise PlanError(
            f"прислано шагов: {len(steps)}, а в утверждённом плане их {was}, "
            "и недостающие пропали бы из него",
            f"передай список целиком, все {was} со статусами — шага, "
            "которого нет в списке, не станет",
        )
