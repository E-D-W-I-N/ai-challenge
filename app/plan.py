...
же сравниваются планы (`advanced`), и забытый там флажок стоил бы обмена."""
...
def _progress(plan) -> tuple:
    """План в виде, которым меряется **продвижение**: у каждого шага одно
    «сделан ли» плюс три флажка. Состав списка держит длина кортежа.
    Заголовок и `in_progress` сюда не попадают намеренно — см. `advanced`."""
    steps = tuple(step.get("status") == "done" for step in _steps(plan))
    return steps, tuple(bool((plan or {}).get(flag)) for flag in FLAGS)


def advanced(before, after) -> bool:
    """Продвинулся ли план — **единственное** место, где планы сравниваются.
    Продвижение: шаг стал `done` или вышел из него, шагов стало больше или
    меньше, поднят или снят флажок. Этим обмен и кончается.

    Взятие шага в работу (`pending` → `in_progress`) продвижением **не**
    считается, и это весь смысл: погаси инструменты на нём — и отметить
    сделанное будет нечем, а модель нарисует отметку словами. Смена этапа
    здесь частный случай: этап вычисляется из плана, и ни одно его условие
    (флажки, число шагов, «все ли done») мимо этого кортежа не проходит."""
    return _progress(before) != _progress(after)
...
STAGE_RULES = {
    ...
    "execution": (
        ...
        "а следующему in_progress, всё через update_plan. План меняется "
        "только вызовом update_plan: написать в ответе «отметил шаг» "
        "или нарисовать список со статусами — значит не изменить ничего. "
        "Не переходи к следующему, пока текущий не сделан. Когда все шаги "
        "done — проверь работу и позови finish_task."
    ),
    "validation": (
