...
class AgentSpec:
    ...
    app/llm.py — можно дополнить, переопределять не нужно. `plugins` там же
    мержатся по `id`: выключенный `context-compression` уцелеет рядом с вашим
    плагином, а названный вами тот же `id` победит.
    """
