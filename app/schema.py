...
class AgentSpec:
    ...
    Обычному чату не нужны — его системная часть лежит в `system`, — но агента
    можно завести и с готовой заготовкой диалога, например с парой примеров
    «вопрос — ответ».
    """
    ...
    """Сколько последних сообщений уходит в окно запроса.

    None — дефолт агента, 0 — агент без памяти. Считается в сообщениях,
    а не в парах: 8 — это четыре обмена. Это лимит **окна**, а не хранилища:
    в истории остаётся всё до `app.agent.MAX_STORED_MESSAGES`.
    """
