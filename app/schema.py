...
class AgentSpec:
    ...
    # делать», а не «делать с нулём». `keep_last is None` значит, что сжатия
    # нет вовсе и в модель уезжает вся история, как в Дне 8. Булева
    # переключателя нет намеренно: в Python True — это int, и разбор полей
    # его отбрасывает (`_optional_field`, app/main.py).
    keep_last: int | None = None
    """Сколько последних реплик уезжает в модель как есть, без сводки."""
...
CONTEXT_FIELDS = ("keep_last", "compress_every")
