"""Каталог моделей OpenRouter: /api/v1/models, ключа не требует.

Отдаётся целиком и со всеми данными о модели: по ним панель предупреждает,
что заданный параметр модель не потянет, — иначе
...
# Обрезают температуру на 1.0 и возвращают 400 на 1.2, но перечисляют
# "temperature" в supported_parameters: по нему о потолке не узнать.
