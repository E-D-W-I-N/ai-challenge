...
class Store:
    ...
    def save_history(self, session_id: str, turns) -> None:
        ...
        Номера расставляются заново от нуля: откат обмена, снятая
        перегенерацией пара и кап хранимого укорачивают историю, и «дописать
        хвост» оставил бы дыры в нумерации.
