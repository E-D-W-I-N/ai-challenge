...
-- нет: этап переключает человек нажатием на полосу, к модели этот переход
-- не ходит, и своей цены у него нет.
--
...
class Store:
    ...
    def _migrate(self) -> list[str]:
        ...
        состояние некому, а платить за переход нечем: этап переключает
        человек нажатием. Взамен цены журнал получает `ok`: состоялся
        переход или его отбила карта.
        """
        done: list[str] = []
        with self.tx() as conn:
            ...
            # Вызова не стало — этап переключает человек нажатием на полосу,
