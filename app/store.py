...
Десять таблиц, их состав и инварианты — таблицей ниже. Сквозного здесь два:
...
# Таблица — что в ней — чем нельзя жертвовать:
#
#   sessions        чат: имя, конфиг         конфиг одним JSON-полем: новое поле
#                                            `AgentSpec` сохраняется само
#   messages        реплики диалога          PK `(session_id, seq)`; пишется целиком
#                                            одной `tx()`, `seq` от нуля без дыр
#   meta            счётчики на всю базу     только растут: номер не выдаётся дважды
#   summaries       сводки начала разговора  не строка в `messages` — её стёр бы
#                                            `DELETE`; чистить на всех трёх путях
#   working_memory  записи о задаче, от руки `seq` AUTOINCREMENT, номера устойчивы;
#                                            индекс по чату; все три пути
#   branches        чей потомок и сколько    PK `session_id`; удаление родителя ветку
#                   сообщений унёс           не удаляет; пути очистки два, не три
#   task_state      три флажка задачи        живут и у пустого списка шагов
#   task_steps      шаги со статусами        `seq` от нуля без дыр: список приезжает
#                                            целиком; все три пути
#   memory          долговременная память    без `session_id`; `seq` AUTOINCREMENT;
#                                            чистит только `clear()`
#   profile         как отвечать человеку    без `session_id`; строка на поле;
#                                            чистит только `clear()`
#
# Схема накатывается одним `CREATE TABLE IF NOT EXISTS`, миграций нет: новая
# таблица заводится сама, колонка в живую — нет. Каскадов и FK нет нигде,
# поэтому «чистить руками» выше — не присказка, а условие.
