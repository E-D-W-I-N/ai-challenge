...
-- незачем.
--
-- `upto` — сколько первых реплик истории уже прочитано извлечением. Без него
-- переключение на `facts` посреди разговора (или подъём чата из базы) резало бы
-- окном начало, которого извлечение никогда не видело.
--
