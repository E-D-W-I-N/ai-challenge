...
* `meta` — счётчики, общие на всю базу. Сейчас там один: номер, из которого
  собирается имя нового чата. Ему мало жить в процессе — после перезапуска
  счёт начался бы заново и выдал бы имя, которое уже занято.
...
Пять секунд с запасом покрывают любую запись сервера — самая долгая, спавн сотни
сессий, укладывается в единицы миллисекунд. Ждать дольше нельзя: ожидание
блокировки блокирует и цикл событий, так что сервер на это время замирает.
"""
...
class Store:
    ...
    блокировки там, где восемь чатов пишут одновременно. Все запросы
    идут под общим `RLock`: писать могут и цикл событий, и рабочий
    поток TestClient.
    """
    ...
    def init(self) -> "Store":
        ...
        if self.path != MEMORY:
            ...
            # «unable to open database file», и сервер не поднимается вовсе.
    ...
    def list_sessions(self, *, limit: int | None = None) -> list[dict]:
        ...
        здесь резала бы по `updated_at`, то есть первыми выпали бы чаты,
        с которыми давно не говорили, — а список единственный путь к диалогу.
        Запрос дешёвый: пять тысяч сессий читаются за 13 мс.
        """
        sql = """
            SELECT s.*, (
    ...
    def clear(self) -> None:
        ...
        Именно целиком: в `meta` лежит счётчик имён, и оставить его значило бы
        отдать следующей проверке пустую базу с номером посередине.
        """
        with self.tx() as conn:
    ...
    def next_counter(self, key: str) -> int:
        ...
        такой уже есть. А консоль, запущенная рядом с сервером, вела бы свой
    ...
    def claim_agent_id(self) -> str:
        ...
        CLI рядом с поднятым сервером), `INSERT` падает на конфликте, счётчик
