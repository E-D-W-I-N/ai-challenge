...
* **`redact()` на всех колонках сразу**: транзакция отдаёт не соединение,
  ...
  колонку нельзя;
* **схема накатывается одним `CREATE TABLE IF NOT EXISTS`, и догонять её
  нечем**: старых баз в продукте нет — их удалили, — и миграций он не ведёт.
  Новая таблица заводится сама; колонка в живую таблицу так не добавится,
  и менять состав колонок можно только вместе со сносом файла базы.
"""
...
-- одна, и поле перестало различать что бы то ни было.
--
